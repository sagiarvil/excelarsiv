#!/usr/bin/env python3
"""
Amortisman + 2026 Yeniden Değerleme — üretim betiği (A1 / manda v6, Hat C C-08).
Ayırt edici: amortisman cetveli + 298/Ç / Geç.32 tek karar ekranı.
"""
...
URUN_AD = "Amortisman + 2026 Yeniden Değerleme"
SURUM = "1.0.0"
RENK = "0F2742"
...
def _kim(ws, hucre, baslik, mesaj):
    yorum_ekle(
        ...
        f"Yanlışsa: Amortisman cetveli ve karar bozulur | Kaynak: VUK 298/Ç · Geçici 32",
    )


def kural_cek(kural_id: str) -> str:
    """Oranları tblKurallar'dan çeker; yıl seçimi ayd_yilTaban ile (G07: adında rakam yok)."""
    return (
        f'=IFERROR(INDEX(CHOOSE(MAX(1,MIN(3,IF(OR(hesapYili="",hesapYili=0),1,'
        f'hesapYili-ayd_yilTaban))),'
...
_YV = "MAX(0,MIN(ayd_yuvarMax,IFERROR(N(G12),0)))"
...
def amortisman(ws):
    sayfa_hazirla(ws, "AMORTISMAN", "1F4E79", URUN_AD, son_kolon=30)
    h(ws, 3, 1, "Amortisman cetveli — yeniden değerleme öncesi / sonrası",
      kalin=True, boyut=13, yazi=KOYU_LACIVERT, kaydir=True)
    ws.merge_cells("A3:J3")
    h(ws, 4, 1,
      "Cetvel, GIRDI maliyet/birikmiş/endeks/kalan ömür ve MOTOR farkından üretilir. "
      "298/Ç ile Geç.32 kararı PANO'da; burada yıllık amortisman etkisi görülür.",
      yazi=GRİ, kaydir=True)
    ws.merge_cells("A4:J4")

    # Özet KPI
    h(ws, 6, 1, "Özet", kalin=True, boyut=12, yazi=KOYU_LACIVERT)
    ozet = [
        (7, "Net defter (önce) [₺]", "=IFERROR(ayd_netDefter,0)", TL),
        (8, "Değer artış farkı [₺]", "=IFERROR(ayd_fark,0)", TL),
        (9, "Yıllık amortisman önce [₺]",
         "=IFERROR(IF(OR(GIRDI!B12=\"\",GIRDI!B12=0),0,ayd_netDefter/GIRDI!B12),0)", TL),
        (10, "Yıllık amortisman sonra [₺]",
         "=IFERROR(IF(OR(GIRDI!B12=\"\",GIRDI!B12=0),0,(ayd_netDefter+ayd_fark)/GIRDI!B12),0)", TL),
        (11, "Yıllık amortisman farkı [₺]", "=IFERROR(B10-B9,0)", TL),
    ]
    for satir, et, form, sayi in ozet:
        h(ws, satir, 1, et, yazi="333333")
        h(ws, satir, 2, form, sayi=sayi, yazi=DIS_REF_YEŞIL, kalin=True)
        _kim(ws, f"B{satir}", et, "Amortisman özeti; GIRDI ve MOTOR'dan gelir")

    h(ws, 12, 1, "Cetvel özeti (kanıt)", kalin=True, yazi=KOYU_LACIVERT)
    h(ws, 12, 11,
      '=_xlfn.TEXTJOIN(" · ",TRUE,"Önce=",TEXT(B9,"₺ #,##0"),"Sonra=",TEXT(B10,"₺ #,##0"),'
      '"Fark=",TEXT(B11,"₺ #,##0"),"Yıl=",TEXT(IFERROR(GIRDI!B12,0),"0"))',
      yazi=DIS_REF_YEŞIL, kaydir=True)
    # ayd_amortismanCetvel → AMORTISMAN!$K$12

    # Yıl cetveli tablosu
    h(ws, 14, 1, "Yıllık amortisman cetveli", kalin=True, boyut=12, yazi=KOYU_LACIVERT)
    sutunlar = [
        "Yil", "AmortOnce", "AmortSonra", "AmortFark",
        "KumulatifOnce", "KumulatifSonra", "KalanNDDOnce", "KalanNDDSonra", "Durum",
    ]
    baslik_satiri(ws, 15, sutunlar)
    formuller = {
        "AmortSonra": (
            '=IF(OR(tblAmortisman[[#This Row],[Yil]]="",'
            'tblAmortisman[[#This Row],[Yil]]>IFERROR(GIRDI!B12,0)),"",'
            'IFERROR(IF(OR(GIRDI!B12="",GIRDI!B12=0),0,(ayd_netDefter+ayd_fark)/GIRDI!B12),0))'
        ),
        "AmortFark": (
            '=IF(tblAmortisman[[#This Row],[AmortOnce]]="","",'
            'IFERROR(tblAmortisman[[#This Row],[AmortSonra]]-'
            'tblAmortisman[[#This Row],[AmortOnce]],0))'
        ),
        "KumulatifOnce": (
            '=IF(tblAmortisman[[#This Row],[AmortOnce]]="","",'
            'IFERROR(tblAmortisman[[#This Row],[Yil]]*tblAmortisman[[#This Row],[AmortOnce]],0))'
        ),
        "KumulatifSonra": (
            '=IF(tblAmortisman[[#This Row],[AmortSonra]]="","",'
            'IFERROR(tblAmortisman[[#This Row],[Yil]]*tblAmortisman[[#This Row],[AmortSonra]],0))'
        ),
        "KalanNDDOnce": (
            '=IF(tblAmortisman[[#This Row],[AmortOnce]]="","",'
            'IFERROR(MAX(0,ayd_netDefter-tblAmortisman[[#This Row],[KumulatifOnce]]),0))'
        ),
        "KalanNDDSonra": (
            '=IF(tblAmortisman[[#This Row],[AmortSonra]]="","",'
            'IFERROR(MAX(0,ayd_netDefter+ayd_fark-tblAmortisman[[#This Row],[KumulatifSonra]]),0))'
        ),
        "Durum": (
            '=IF(tblAmortisman[[#This Row],[Yil]]="","",'
            'IF(tblAmortisman[[#This Row],[Yil]]>IFERROR(GIRDI!B12,0),"DIŞI","AKTİF"))'
        ),
    }
    formuller["AmortOnce"] = (
        '=IF(OR(tblAmortisman[[#This Row],[Yil]]="",'
        'tblAmortisman[[#This Row],[Yil]]>IFERROR(GIRDI!B12,0)),"",'
        'IFERROR(IF(OR(GIRDI!B12="",GIRDI!B12=0),0,ayd_netDefter/GIRDI!B12),0))'
    )
    tablo_ekle(ws, "tblAmortisman", "A15:I1014", sutunlar, formuller)
    # Ön doldur: 40 yıl satırı (demo 10; kalan boş Yil ile)
    for i in range(1, 41):
        r = 15 + i
        ws.cell(r, 1).value = i
        ws.cell(r, 1).number_format = CATI
        ws.cell(r, 1).fill = PatternFill("solid", start_color=GIRIS_SARI, end_color=GIRIS_SARI)
        ws.cell(r, 1).protection = Protection(locked=False)
        _kim(ws, f"A{r}", "Yıl", "Cetvel yılı; kalan ömür dışındakiler DIŞI olur")

    dogrulama(ws, "whole", "1", "A16:A1014",
              baslik="Yıl", mesaj="1 ile 100 arasında yıl numarası.",
              hata_baslik="Geçersiz yıl", hata_mesaj="1-100 arası girin.",
              isaret="between", f2="100")
    giris_hucreleri(ws, 16, 55, [1])

    # Demo grafik verisi
    h(ws, 15, 11, "OnceDemo", kalin=True, yazi="FFFFFF", zemin=ORTA_MAVI)
    h(ws, 15, 12, "SonraDemo", kalin=True, yazi="FFFFFF", zemin=ORTA_MAVI)
    for i in range(1, 11):
        h(ws, 15 + i, 11, 80000, sayi=TL, yazi=GRİ)
        h(ws, 15 + i, 12, 125000, sayi=TL, yazi=GRİ)

    g1 = BarChart()
    g1.type = "col"
    g1.title = "Amortisman Önce vs Sonra"
    g1.add_data(Reference(ws, min_col=11, min_row=15, max_col=12, max_row=25), titles_from_data=True)
    g1.set_categories(Reference(ws, min_col=1, min_row=16, max_row=25))
    g1.height, g1.width = 8, 14
    ws.add_chart(g1, "K6")

    g2 = LineChart()
    g2.title = "Kümülatif Amortisman Trendi"
    g2.add_data(Reference(ws, min_col=11, min_row=15, max_col=12, max_row=25), titles_from_data=True)
    g2.set_categories(Reference(ws, min_col=1, min_row=16, max_row=25))
    g2.height, g2.width = 8, 12
    ws.add_chart(g2, "K22")

    genislik(ws, {
        "A": 32, "B": 16, "C": 16, "D": 14, "E": 16, "F": 16, "G": 16, "H": 16, "I": 10,
        "K": 18, "L": 14,
    })
    sabitle(ws, "A16")
    alt_bant(ws, 1016, "Amortisman cetveli kilitlidir; yıl numarası sarı alana girilir.")
...
def girdi(ws):
    ...
    h(ws, 17, 2,
      '=IFERROR(IF(OR(COUNTA(B6:B13)=0,ayd_girisBeklenen=0),0,'
      'ROUND(COUNTA(B6:B13)/ayd_girisBeklenen*100,0)),0)',
...
def kurallar(ws):
    ...
    satirlar = [
        ["AYD-001", "VUK 298/Ç", "endeks_taban", "endeks>=taban", "endeks_taban",
         1.0, 1.0, 1.0, "01.01.2025", "VUK md.298/Ç"],
        ["AYD-002", "Geçici 32", "gec32_fon_orani", "fark>0", "gec32_fon_orani",
         0.02, 0.02, 0.02, "01.01.2025", "VUK Geçici md.32"],
        ["AYD-003", "KVK md.32", "kv_oran", "vergi etkisi", "kv_oran",
         0.25, 0.25, 0.25, "01.01.2025", "KVK md.32"],
        ["AYD-004", "VUK 298/Ç", "c298_fon_orani", "298 yolu", "c298_fon_orani",
         0.0, 0.0, 0.0, "01.01.2025", "VUK md.298/Ç"],
        ["AYD-005", "Genel", "yuvarlama", "her hesap", "yuvarlama_ondalik",
        ...
        for k, v in enumerate(s, 1):
            ...
            if k in (6, 7, 8):
                if s[0] == "AYD-005":
                    ...
                    sayi = "0.00"
                elif s[0] in ("AYD-002", "AYD-003", "AYD-004"):
                    sayi = YÜZDE
    ...
    formuller = {
        "aktif_deger": (
            ...
            "IFERROR(CHOOSE(MAX(1,MIN(3,IF(OR(hesapYili=\"\",hesapYili=0),1,hesapYili-ayd_yilTaban))),"
    ...
    h(ws, 15, 2, kural_cek("AYD-002"), sayi=YÜZDE, yazi=DIS_REF_YEŞIL)
    h(ws, 16, 1, "KV oranı (aktif yıl)", yazi="333333")
    h(ws, 16, 2, kural_cek("AYD-003"), sayi=YÜZDE, yazi=DIS_REF_YEŞIL)
    h(ws, 17, 1, "Matris metni", yazi="333333")
    h(ws, 17, 2,
      '=_xlfn.TEXTJOIN(" | ",TRUE,"AYD-002=",TEXT(B15,"0.0%"),"AYD-003=",TEXT(B16,"0.0%"),"yıl=",hesapYili)',
      yazi=DIS_REF_YEŞIL, kaydir=True)
...
def motor(ws):
    ...
    ekle("Hesap yılı oku", "=hesapYili", "yıl", "Aktif mevzuat yılı", "AYD-005")  # G7
    ekle("Endeks taban", km("AYD-001"), "çarpan", "Minimum endeks", "AYD-001")  # G8
    ekle("Geç.32 fon oranı", km("AYD-002"), "oran", "Fon vergi oranı", "AYD-002")  # G9
    ekle("KV oranı", km("AYD-003"), "oran", "Kurumlar vergisi oranı", "AYD-003")  # G10
    ekle("298/Ç fon oranı", km("AYD-004"), "oran", "298 yolu fon", "AYD-004")  # G11
    ekle("Yuvarlama ondalık", km("AYD-005"), "adet", "Yuvarlama basamağı", "AYD-005")  # G12
    ekle("Maliyet", "=GIRDI!B9", "TL", "İktisap maliyeti", "AYD-001")  # G13
    ekle("Birikmiş amortisman", "=GIRDI!B10", "TL", "Birikmiş amortisman", "AYD-001")  # G14
    ekle("Endeks oranı", "=GIRDI!B11", "çarpan", "Kullanıcı endeksi", "AYD-001")  # G15
    ekle("Kalan ömür", "=GIRDI!B12", "yıl", "Kalan ekonomik ömür", "AYD-003")  # G16
    ekle("Net defter değeri", "=MAX(0,G13-G14)", "TL", "Maliyet − birikmiş", "AYD-001")  # G17
    ekle("Endeks düzeltilmiş", "=MAX(G15,G8)", "çarpan", "Taban ile max", "AYD-001")  # G18
    ekle("Yeniden değerlenmiş maliyet", f"=ROUND(G13*G18,{_YV})", "TL", "Maliyet × endeks", "AYD-001")  # G19
    ekle("Değer artış farkı", f"=ROUND(MAX(0,G13*(G18-1)),{_YV})", "TL", "Maliyet×(endeks−1)", "AYD-001")  # G20
    ekle("Geç.32 fon vergisi", f"=ROUND(G20*G9,{_YV})", "TL", "Fark × fon oranı", "AYD-002")  # G21
    ekle("298/Ç fon vergisi", f"=ROUND(G20*G11,{_YV})", "TL", "Fark × 298 fon", "AYD-004")  # G22
    ekle("Özkaynak 298/Ç", "=G20-G22", "TL", "Fark − 298 fon", "AYD-004")  # G23
    ekle("Özkaynak Geç.32", "=G20-G21", "TL", "Fark − Geç.32 fon", "AYD-002")  # G24
    ekle("Ek amortisman yıllık", "=IF(G16=0,0,G20/G16)", "TL", "Fark / kalan ömür", "AYD-003")  # G25
    ekle("Vergi tasarrufu brüt", f"=ROUND(G20*G10,{_YV})", "TL", "Fark × KV oranı", "AYD-003")  # G26
    ekle("Net fayda 298/Ç", "=G23", "TL", "298 özkaynak net", "AYD-004")  # G27
    ekle("Net fayda Geç.32", "=G24+G26*ayd_tasarrufCarpan", "TL", "Özkaynak+vergi faydası", "AYD-002")  # G28
    ekle("Fayda farkı (298−Geç32)", "=G27-G28", "TL", "Yol karşılaştırması", "AYD-001")  # G29
    ekle("Karar kodu",
         '=IF(COUNTA(tblGirdi[Alan Anahtarı])=0,"VERI_YOK",'
         'IF(G20<=0,"YAPMA",'
         'IF(ABS(G29)<=ayd_kararEsik,"BEKLE","YAP")))',
         "kod", "Karar kodu", "AYD-001")  # G30
    ekle("Karar metni",
         '=IF(G30="VERI_YOK","VERİ YOK",'
         'IF(G30="YAPMA","YAPMA",'
         'IF(G30="BEKLE","BEKLE","YAP")))',
         "metin", "Kullanıcıya karar", "AYD-001")  # G31
    ekle("Gerekçe",
         '=IF(G30="VERI_YOK","Giriş tablosu boş — amortisman ve yeniden değerleme hesaplanamaz.",'
         'IF(G30="YAPMA","Endeks artışı yok veya fark sıfır; yeniden değerleme yapılmamalı.",'
         'IF(G30="BEKLE","298/Ç ve Geç.32 net faydaları eşik içinde; öncelik yorumunu ve amortisman cetvelini gözden geçirin.",'
         'IF(G29>0,"YAP · 298/Ç net faydası önde; amortisman cetveli ve fon yükü uygun.",'
         '"YAP · Geç.32 net faydası (özkaynak+vergi) önde; amortisman cetveli ile birlikte uygulayın."))))',
         "metin", "Gerekçe cümlesi", "AYD-001")  # G32
    ekle("Fark / maliyet", "=IF(G13=0,0,G20/G13)", "oran", "Artış yoğunluğu", "AYD-001")  # G33
    ekle("Fon / fark", "=IF(G20=0,0,G21/G20)", "oran", "Fon yük oranı", "AYD-002")  # G34
    ekle("Yorum A net (298 öncelik)", "=G27", "TL", "A yolu net", "AYD-004")  # G35
    ekle("Yorum B net (Geç.32 öncelik)", "=G28", "TL", "B yolu net", "AYD-002")  # G36
    ekle("Seçili yorum net",
         '=IF(GIRDI!B13="B",G36,G35)',
         "TL", "A/B seçimine göre", "AYD-001")  # G37
    ekle("Yorum farkı", "=ABS(G35-G36)", "TL", "Belirsizlik aralığı", "AYD-001")  # G38
    ekle("Güven skoru",
         "=IFERROR(ROUND(IF(OR(COUNTA(GIRDI!B6:B13)=0,ayd_girisBeklenen=0),0,"
         "COUNTA(GIRDI!B6:B13)/ayd_girisBeklenen*100),0),0)",
         "puan", "Giriş bütünlüğü", "AYD-005")  # G39
    ekle("Risk skoru",
         '=IF(G30="VERI_YOK",0,IF(G30="YAPMA",ayd_riskDusuk,'
         'IF(G30="BEKLE",ayd_riskOrta,ayd_riskYuksek)))',
         "puan", "Karar riski", "AYD-001")  # G40
    ekle("Senaryo iyimser maliyet", "=G13*ayd_senaryoIyi", "TL", "İyimser maliyet", "AYD-001")  # G41
    ekle("Senaryo kötümser maliyet", "=G13*ayd_senaryoKotu", "TL", "Kötümser maliyet", "AYD-001")  # G42
    ekle("İyimser fark", f"=ROUND(MAX(0,G41*(G18-1)),{_YV})", "TL", "İyimser fark", "AYD-001")  # G43
    ekle("Kötümser fark", f"=ROUND(MAX(0,G42*(G18-1)),{_YV})", "TL", "Kötümser fark", "AYD-001")  # G44
    ekle("M-SEN fon+1 puan fark net",
         f"=ROUND(G20*(G9+ayd_oranArtisPuan),{_YV})",
         "TL", "Fon +1 puan senaryosu", "AYD-002")  # G45
    ekle("M-SEN özkaynak Geç.32", "=G20-G45", "TL", "M-SEN net özkaynak", "AYD-002")  # G46
    ekle("Tahmin üst", "=G20*ayd_tahminUst", "TL", "Tahmin üst bant", "AYD-005")  # G47
    ekle("Tahmin alt", "=G20*ayd_tahminAlt", "TL", "Tahmin alt bant", "AYD-005")  # G48
    ekle("Tornado: maliyet etkisi",
         f"=ABS(ROUND(MAX(0,(G13*ayd_tornadoMaliyet)*(G18-1)),{_YV})-G20)",
         "TL", "Maliyet ± etki", "AYD-001")  # G49
    ekle("Tornado: endeks etkisi",
         f"=ABS(ROUND(MAX(0,G13*(G18*ayd_tornadoEndeks-1)),{_YV})-G20)",
         "TL", "Endeks ± etki", "AYD-001")  # G50
    ekle("Tornado: fon etkisi",
         f"=ABS(ROUND(G20*(G9+ayd_oranArtisPuan),{_YV})-G21)",
         "TL", "Fon ± etki", "AYD-002")  # G51
    ekle("Madde atıf özeti",
         '=_xlfn.TEXTJOIN(" | ",TRUE,"AYD-001→VUK 298/Ç","AYD-002→Geçici 32","AYD-003→KVK md.32")',
         "metin", "Kanıt atıfları", "AYD-001")  # G52
    ekle("Kanıt satır özeti",
         '=_xlfn.TEXTJOIN(" · ",TRUE,"Fark=",TEXT(G20,"₺ #,##0"),'
         '"Fon32=",TEXT(G21,"₺ #,##0"),"Özkaynak298=",TEXT(G23,"₺ #,##0"))',
         "metin", "Rapor özeti", "AYD-005")  # G53
...
def senaryo(ws):
    ...
    fon = 'IFERROR(INDEX(tblKurallar[aktif_deger],MATCH("AYD-002",tblKurallar[kural_id],0)),0)'
    senaryolar = [
        ("İyimser", "=IFERROR(ayd_senaryoIyi+N(GIRDI!B9)*0,0)",
         "=IFERROR(ROUND(MAX(0,IFERROR(ayd_maliyet,0)*B6*(IFERROR(GIRDI!B11,1)-1)),2),0)",
         f"=IFERROR(ROUND(C6*{fon},2),0)",
         '=IF(COUNTA(tblGirdi[Alan Anahtarı])=0,"VERİ YOK",IF(C6=0,"YAPMA","YAP"))'),
        ("Baz", "=IFERROR(1+N(GIRDI!B9)*0,1)",
         "=IFERROR(ROUND(MAX(0,IFERROR(ayd_maliyet,0)*B7*(IFERROR(GIRDI!B11,1)-1)),2),0)",
         f"=IFERROR(ROUND(C7*{fon},2),0)",
         '=IF(COUNTA(tblGirdi[Alan Anahtarı])=0,"VERİ YOK",IF(C7=0,"YAPMA","YAP"))'),
        ("Kötümser", "=IFERROR(ayd_senaryoKotu+N(GIRDI!B9)*0,0)",
         "=IFERROR(ROUND(MAX(0,IFERROR(ayd_maliyet,0)*B8*(IFERROR(GIRDI!B11,1)-1)),2),0)",
         f"=IFERROR(ROUND(C8*{fon},2),0)",
         '=IF(COUNTA(tblGirdi[Alan Anahtarı])=0,"VERİ YOK",IF(C8=0,"YAPMA","YAP"))'),
        ("M-SEN fon+1 puan", "=IFERROR(1+N(GIRDI!B9)*0,1)",
         "=IFERROR(ayd_fark,0)",
         "=IFERROR(MOTOR!G45,0)",
         '=IF(COUNTA(tblGirdi[Alan Anahtarı])=0,"VERİ YOK",IF(C9=0,"YAPMA",IF(ABS(D9-MOTOR!G21)>ayd_kararEsik,"BEKLE","YAP")))'),
    ]
    ...
    h(ws, 25, 3, "=ayd_fark", sayi=TL, yazi=DIS_REF_YEŞIL)
...
def vakalar(ws):
    ...
    vakalar_data = [
        ("YD-V01", "Tebliğ örnek 1 — pozitif fark",
         ...
         450000,
         "=IFERROR(ROUND(MAX(0,vaka_maliyet_1*(vaka_endeks_1-INDEX(tblKurallar[deger_2025],MATCH(\"AYD-001\",tblKurallar[kural_id],0)))),2),0)",
         "=IFERROR(D6-E6,0)",
         '=IFERROR(IF(ABS(F6)<=ayd_tolerans,"TUTARLI","KIRIK"),"KIRIK")',
         "VUK 298/Ç · Geçici 32 hesap örneği"),
        ("YD-V02", "Tebliğ örnek 2 — orta endeks",
         "maliyet=vaka_maliyet_2; endeks=vaka_endeks_2",
         400000,
         "=IFERROR(ROUND(MAX(0,vaka_maliyet_2*(vaka_endeks_2-INDEX(tblKurallar[deger_2025],MATCH(\"AYD-001\",tblKurallar[kural_id],0)))),2),0)",
         "=IFERROR(D7-E7,0)",
         '=IFERROR(IF(ABS(F7)<=ayd_tolerans,"TUTARLI","KIRIK"),"KIRIK")',
         "VUK 298/Ç hesap örneği"),
        ("YD-V03", "Tebliğ örnek 3 — düşük endeks",
         "maliyet=vaka_maliyet_3; endeks=vaka_endeks_3",
         50000,
         "=IFERROR(ROUND(MAX(0,vaka_maliyet_3*(vaka_endeks_3-INDEX(tblKurallar[deger_2025],MATCH(\"AYD-001\",tblKurallar[kural_id],0)))),2),0)",
         "=IFERROR(D8-E8,0)",
         '=IFERROR(IF(ABS(F8)<=ayd_tolerans,"TUTARLI","KIRIK"),"KIRIK")',
         "VUK 298/Ç hesap örneği"),
        ("YD-V04", "Geç.32 fon kontrolü",
         "fark=vaka_maliyet_1*(vaka_endeks_1-1); fon oranı AYD-002",
         9000,
         "=IFERROR(ROUND(MAX(0,vaka_maliyet_1*(vaka_endeks_1-1))*INDEX(tblKurallar[deger_2025],MATCH(\"AYD-002\",tblKurallar[kural_id],0)),2),0)",
         "=IFERROR(D9-E9,0)",
         '=IFERROR(IF(ABS(F9)<=ayd_tolerans,"TUTARLI","KIRIK"),"KIRIK")',
         "VUK Geçici md.32"),
    ]
    ...
    formuller = {
        ...
        "durum": '=IF(tblVakalar[[#This Row],[vaka_id]]="","",IFERROR(IF(ABS(tblVakalar[[#This Row],[fark]])<=ayd_tolerans,"TUTARLI","KIRIK"),"KIRIK"))',
    }
...
def kanit_raporu(ws):
    ...
    h(ws, 3, 1, "KANIT RAPORU — Amortisman + 2026 Yeniden Değerleme", kalin=True, boyut=14, yazi=KOYU_LACIVERT, kaydir=True)
    ...
    for i, (et, form, sayi) in enumerate([
        ...
        ("Değer artış farkı", "=ayd_fark", TL),
        ("Net defter", "=ayd_netDefter", TL),
    ], 9):
    ...
    h(ws, 17, 2, "=ayd_fonGec32", sayi=TL, yazi=DIS_REF_YEŞIL, kalin=True)
    h(ws, 18, 1, "Özkaynak 298/Ç", yazi="333333")
    h(ws, 18, 2, "=ayd_ozKaynak298", sayi=TL, yazi=DIS_REF_YEŞIL, kalin=True)
    h(ws, 19, 1, "KARAR", kalin=True, yazi=KOYU_LACIVERT)
    h(ws, 19, 2, "=ayd_kararMetni", kalin=True, boyut=12, yazi=DIS_REF_YEŞIL)
    h(ws, 20, 1, "Gerekçe", yazi="333333")
    h(ws, 20, 2, "=ayd_kararGerekce", yazi=DIS_REF_YEŞIL, kaydir=True)
    ws.merge_cells("B20:F20")

    h(ws, 22, 1, "Madde atıfları", kalin=True, yazi=KOYU_LACIVERT)
    h(ws, 22, 2, "=ayd_maddeAtifMetni", yazi=DIS_REF_YEŞIL, kaydir=True)
    ws.merge_cells("B22:F22")

    h(ws, 24, 1, "Kanıt gövdesi", kalin=True, yazi=KOYU_LACIVERT)
    h(ws, 24, 2, "=ayd_kanitRaporu", yazi=DIS_REF_YEŞIL, kaydir=True)
    ws.merge_cells("B24:F24")

    h(ws, 26, 1, "Parmak izi (SHA-256 kısaltma)", yazi="333333")
    h(ws, 26, 2, "=ayd_parmakIzi", yazi=GRİ)
...
def pano(ws):
    ...
    h(ws, 3, 1, "Yönetim paneli — Amortisman + YD", kalin=True, boyut=14, yazi=KOYU_LACIVERT)

    kpiler = [
        (2, "Maliyet", "=IFERROR(ayd_maliyet,0)", TL),
        (3, "Fark", "=IFERROR(ayd_fark,0)", TL),
        (4, "Fon Geç.32", "=IFERROR(ayd_fonGec32,0)", TL),
        (5, "Özkaynak 298", "=IFERROR(ayd_ozKaynak298,0)", TL),
        ...
        (11, "M-SEN fon", "=IFERROR(ayd_kuralDegisimSenaryo,0)", TL),
        (12, "Tahmin", "=IFERROR(SENARYO!B32,0)", TL),
        (13, "Amort fark", "=IFERROR(AMORTISMAN!B11,0)", TL),
    ]
    ...
    h(ws, 6, 2, "=ayd_kararMetni", boyut=14, kalin=True, yazi=DIS_REF_YEŞIL)
    h(ws, 7, 1, "Gerekçe", kalin=True, yazi=KOYU_LACIVERT)
    h(ws, 7, 2, "=ayd_kararGerekce", yazi=DIS_REF_YEŞIL, kaydir=True)
    ...
    moduller = [
        ("T1", "Fark / maliyet",
         "=IFERROR(IF(ayd_maliyet=0,0,ayd_fark/ayd_maliyet),0)",
         '=_xlfn.TEXTJOIN("; ",TRUE,"Artış oranı ",TEXT(C11,"0.0%")," — endeks gücünü gösterir")'),
        ("T2", "Fon / fark",
         "=IFERROR(IF(ayd_fark=0,0,ayd_fonGec32/ayd_fark),0)",
        ...
        ("O2", "Tornado max etki",
         "=IFERROR(ayd_duyarlilikSira,\"-\")",
         "=IFERROR(ayd_yorumDuyarlilik,\"-\")"),
        ("O3", "Senaryo bant",
         "=IFERROR(ayd_senaryoKarsilastirma,0)",
         "=IFERROR(ayd_yorumSenaryo,\"-\")"),
        ("O6", "298 vs Geç.32 özkaynak",
         "=IFERROR(IF(ayd_ozKaynak298=0,0,MOTOR!G24/ayd_ozKaynak298),0)",
        ...
        ("M", "M-SEN fon",
         "=IFERROR(ayd_kuralDegisimSenaryo,0)",
         "=IFERROR(ayd_yorumKuralDegisim,\"-\")"),
        ("I1", "Tahmin aralık",
         "=IFERROR(ayd_tahminAralik,0)",
         "=IFERROR(ayd_yorumTahmin,\"-\")"),
        ("I2", "Senaryo fark P90",
         "=IFERROR(IF(COUNT(SENARYO!C6:C9)<2,0,_xlfn.PERCENTILE.INC(SENARYO!C6:C9,ayd_yuzdelikOran)),0)",
    ...
    genislik(ws, {"A": 36, "B": 16, "C": 18, "D": 55})
...
def kontroller(ws):
    ...
    kontroller_list = [
        ...
        ("Hesap yılı geçerli mi?",
         '=IFERROR(IF(AND(N(hesapYili)>N(ayd_yilTaban),N(hesapYili)<=N(ayd_yilTaban)+3),"TEMİZ","HATALI"),"HATALI")'),
        ("Fark sıfır mı?",
         '=IF(IFERROR(ayd_fark,0)=0,"SIFIR","NORMAL")'),
        ("Vaka kırık mı?",
         '=IFERROR(IF(COUNTIF(tblVakalar[durum],"KIRIK")>0,"KIRIK","TUTARLI"),"KIRIK")'),
        ("Fark eşiği aşıyor mu?",
         '=IF(IFERROR(ayd_fark,0)>ayd_farkEsik,"AŞIYOR","NORMAL")'),
    ...
    h(ws, 14, 2, "=IFERROR(ayd_girisBeklenen+COUNTA(GIRDI!B6:B13)*0,0)", sayi=CATI, yazi=DIS_REF_YEŞIL)
    ...
    h(ws, 17, 2, "=IFERROR(ayd_fark,0)", sayi=TL, yazi=DIS_REF_YEŞIL)
    h(ws, 18, 1, "Fon kontrol", yazi="333333")
    h(ws, 18, 2, "=IFERROR(ayd_fonGec32,0)", sayi=TL, yazi=DIS_REF_YEŞIL)
    h(ws, 19, 1, "Karar kontrol", yazi="333333")
    h(ws, 19, 2, "=IFERROR(ayd_kararMetni,\"-\")", yazi=DIS_REF_YEŞIL)
...
def ornek_veri(ws):
    ...
    formuller = {
        ...
        "Fon Geç.32": (
            ...
            "MATCH(\"AYD-002\",tblKurallar[kural_id],0)),2),0))"
        ),
        "Özkaynak 298": (
            "=IF(tblOrnek[[#This Row],[Senaryo]]=\"\",\"\","
            "IFERROR(tblOrnek[[#This Row],[Fark]]-tblOrnek[[#This Row],[Fark]]*"
            "INDEX(tblKurallar[deger_2025],MATCH(\"AYD-004\",tblKurallar[kural_id],0)),0))"
        ),
    }
...
def ayarlar(ws):
    ...
    baslik_satiri(ws, 5, sutunlar)
    params = [
        ("hesapYili", DEMO["hesapYili"], "yıl", "Aktif hesap yılı", "Kullanıcı / GIRDI", "01.01.2025", "14.08.2026"),
        ("raporTarihi", RAPOR_TARIH, "tarih", "Rapor tarihi (sabit)", "Üretim", "01.01.2025", "14.08.2026"),
        ("ayd_tolerans", 0.01, "TL", "Vaka tutarlılık toleransı", "Uygulama notu", "01.01.2025", "14.08.2026"),
        ("ayd_farkEsik", 100_000, "TL", "PANO fark uyarı eşiği", "İç politika", "01.01.2025", "14.08.2026"),
        ("ayd_senaryoIyi", 0.90, "çarpan", "İyimser maliyet çarpanı", "Model varsayımı", "01.01.2025", "14.08.2026"),
        ("ayd_senaryoKotu", 1.10, "çarpan", "Kötümser maliyet çarpanı", "Model varsayımı", "01.01.2025", "14.08.2026"),
        ("ayd_oranArtisPuan", 0.01, "oran", "M-SEN fon +1 puan", "Senaryo motoru", "01.01.2025", "14.08.2026"),
        ("ayd_tahminAlt", 0.85, "çarpan", "Tahmin alt bant", "Model varsayımı", "01.01.2025", "14.08.2026"),
        ("ayd_tahminUst", 1.15, "çarpan", "Tahmin üst bant", "Model varsayımı", "01.01.2025", "14.08.2026"),
        ("ayd_tornadoMaliyet", 1.05, "çarpan", "Tornado maliyet şoku", "Model varsayımı", "01.01.2025", "14.08.2026"),
        ("ayd_tornadoEndeks", 1.05, "çarpan", "Tornado endeks şoku", "Model varsayımı", "01.01.2025", "14.08.2026"),
        ("ayd_tasarrufCarpan", 0.5, "çarpan", "Vergi tasarrufu ağırlığı", "Belirsizlik A/B", "01.01.2025", "14.08.2026"),
        ("ayd_yuzdelikOran", 0.90, "oran", "Yüzdelik P90", "İstatistik", "01.01.2025", "14.08.2026"),
        ("ayd_parmakIzi", "AYD-PRO-1.0.0", "metin", "Dosya parmak izi etiketi", "Üretim", "01.01.2025", "14.08.2026"),
        ("dosya_surumu", SURUM, "metin", "Ürün sürümü", "ExcelArşiv", "01.01.2025", "14.08.2026"),
        ("ayd_girisBeklenen", 8, "adet", "Zorunlu giriş alanı sayısı", "Ürün mimarisi", "01.01.2025", "14.08.2026"),
        ("ayd_riskDusuk", 15, "puan", "Düşük risk skoru", "İç politika", "01.01.2025", "14.08.2026"),
        ("ayd_riskOrta", 55, "puan", "Orta risk skoru", "İç politika", "01.01.2025", "14.08.2026"),
        ("ayd_riskYuksek", 85, "puan", "Yüksek risk skoru", "İç politika", "01.01.2025", "14.08.2026"),
        ("ayd_kararEsik", 25_000, "TL", "YAP/BEKLE fayda fark eşiği", "İç politika", "01.01.2025", "14.08.2026"),
        ("ayd_yilTaban", 2024, "yıl", "CHOOSE yıl tabanı (hesapYili−taban)", "Ürün mimarisi", "01.01.2025", "14.08.2026"),
        ("ayd_yuvarMax", 10, "adet", "ROUND basamak tavanı", "Ürün mimarisi", "01.01.2025", "14.08.2026"),
        ("vaka_maliyet_1", 1_000_000, "TL", "Vaka1 maliyet", "Tebliğ örneği", "01.01.2025", "14.08.2026"),
        ("vaka_endeks_1", 1.45, "çarpan", "Vaka1 endeks", "Tebliğ örneği", "01.01.2025", "14.08.2026"),
        ("vaka_maliyet_2", 2_000_000, "TL", "Vaka2 maliyet", "Tebliğ örneği", "01.01.2025", "14.08.2026"),
        ("vaka_endeks_2", 1.20, "çarpan", "Vaka2 endeks", "Tebliğ örneği", "01.01.2025", "14.08.2026"),
        ("vaka_maliyet_3", 500_000, "TL", "Vaka3 maliyet", "Tebliğ örneği", "01.01.2025", "14.08.2026"),
        ("vaka_endeks_3", 1.10, "çarpan", "Vaka3 endeks", "Tebliğ örneği", "01.01.2025", "14.08.2026"),
    ]
...
def kilavuz(ws):
    ...
    for i, m in enumerate([
        ...
        "YAP — yeniden değerleme farkı pozitif ve yol farkı eşik dışı; 298/Ç veya Geç.32 uygulanır.",
        "YAPMA — endeks artışı yok veya fark sıfır; yeniden değerleme yapılmamalı.",
        "BEKLE — 298/Ç ve Geç.32 net faydaları eşik içinde; öncelik ve amortisman cetveli gözden geçirilir.",
        "AMORTISMAN — yıllık amortisman cetveli yeniden değerleme öncesi/sonrası yan yanadır.",
    ], 5):
    ...
    h(ws, 18, 1,
      "PANO → GIRDI → AMORTISMAN → KURALLAR → MOTOR → SENARYO → VAKALAR → KANIT_RAPORU → AYARLAR → KILAVUZ.",
      kaydir=True, yazi="333333")
...
def kosullu_bicimlendirme(wb):
    ...
    ws = wb["PANO"]
    ws.conditional_formatting.add("B6", FormulaRule(formula=['B6="YAP"'], font=yesil, fill=y_fill))
    ws.conditional_formatting.add("B6", FormulaRule(formula=['B6="BEKLE"'], font=amber, fill=a_fill))
    ws.conditional_formatting.add("B6", FormulaRule(formula=['OR(B6="YAPMA",B6="VERİ YOK")'], font=kirmizi, fill=k_fill))
    ...
    ws.conditional_formatting.add("E6:E9", FormulaRule(formula=['E6="YAP"'], font=yesil))
    ws.conditional_formatting.add("E6:E9", FormulaRule(formula=['OR(E6="YAPMA",E6="BEKLE",E6="VERİ YOK")'], font=amber))
    ...
    ws.conditional_formatting.add("F7:F60", FormulaRule(formula=['F7="AYD-002"'], font=Font(color="B08948", bold=True)))
    ws.conditional_formatting.add("F7:F60", FormulaRule(formula=['F7="AYD-001"'], font=Font(color=KRITIK, bold=True)))

    ws = wb["AMORTISMAN"]
    ws.conditional_formatting.add("B7:B11", CellIsRule(operator="lessThan", formula=["0"], font=kirmizi))
    ws.conditional_formatting.add("B11", CellIsRule(operator="greaterThan", formula=["0"], font=amber))
    ws.conditional_formatting.add("I16:I55", FormulaRule(formula=['I16="AKTİF"'], font=yesil, fill=y_fill))
    ws.conditional_formatting.add("I16:I55", FormulaRule(formula=['I16="DIŞI"'], font=amber, fill=a_fill))
    for harf in ("B", "C", "D", "E", "F", "G", "H"):
        ws.conditional_formatting.add(f"{harf}16:{harf}55", CellIsRule(operator="lessThan", formula=["0"], font=kirmizi))
    ...
    ws = wb["KANIT_RAPORU"]
    ws.conditional_formatting.add("B19", FormulaRule(formula=['B19="YAP"'], font=yesil, fill=y_fill))
    ws.conditional_formatting.add("B19", FormulaRule(formula=['B19="BEKLE"'], font=amber, fill=a_fill))
    ws.conditional_formatting.add("B19", FormulaRule(formula=['OR(B19="YAPMA",B19="VERİ YOK")'], font=kirmizi, fill=k_fill))
...
def ad_tanimlari(wb):
    ...
    ad_ekle(wb, "ayd_maliyet", "GIRDI!$B$9")
    ad_ekle(wb, "ayd_netDefter", "GIRDI!$B$15")
    ad_ekle(wb, "ayd_fark", "MOTOR!$G$20")
    ad_ekle(wb, "ayd_fonGec32", "MOTOR!$G$21")
    ad_ekle(wb, "ayd_ozKaynak298", "MOTOR!$G$23")
    ad_ekle(wb, "ayd_kararMetni", "MOTOR!$G$31")
    ad_ekle(wb, "ayd_kararGerekce", "MOTOR!$G$32")
    ad_ekle(wb, "ayd_kararKapi", "MOTOR!$G$31")
    ad_ekle(wb, "ayd_amortismanCetvel", "AMORTISMAN!$K$12")
    ad_ekle(wb, "ayd_maddeAtifMetni", "MOTOR!$G$52")
    ad_ekle(wb, "ayd_kanitRaporu", "MOTOR!$G$53")
    ad_ekle(wb, "ayd_kuralYilMatris", "KURALLAR!$B$17")
    ad_ekle(wb, "ayd_parmakIzi", "AYARLAR!$B$19")

    ad_ekle(wb, "ayd_senaryoKarsilastirma", "SENARYO!$B$11")
    ad_ekle(wb, "ayd_yorumSenaryo", "SENARYO!$B$12")
    ad_ekle(wb, "ayd_duyarlilikSira", "SENARYO!$B$19")
    ad_ekle(wb, "ayd_yorumDuyarlilik", "SENARYO!$B$20")
    ad_ekle(wb, "ayd_kuralDegisimSenaryo", "SENARYO!$B$22")
    ad_ekle(wb, "ayd_yorumKuralDegisim", "SENARYO!$B$23")
    ad_ekle(wb, "ayd_tahminAralik", "SENARYO!$B$32")
    ad_ekle(wb, "ayd_yorumTahmin", "SENARYO!$B$33")

    ad_ekle(wb, "ayd_vakaDurum", "VAKALAR!$B$12")
    ad_ekle(wb, "ayd_vakaFark", "VAKALAR!$B$13")
    ...
    ayar_map = {
        ...
        "ayd_senaryoIyi": 10,
        "ayd_senaryoKotu": 11,
        "ayd_oranArtisPuan": 12,
        "ayd_tahminAlt": 13,
        "ayd_tahminUst": 14,
        "ayd_tornadoMaliyet": 15,
        "ayd_tornadoEndeks": 16,
        "ayd_tasarrufCarpan": 17,
        "ayd_yuzdelikOran": 18,
        "ayd_girisBeklenen": 21,
        "ayd_riskDusuk": 22,
        "ayd_riskOrta": 23,
        "ayd_riskYuksek": 24,
        "ayd_kararEsik": 25,
    ...
    modul_map = {
        "ayd_modulT1": (11, "C"), "ayd_modulT1Yorum": (11, "D"),
        "ayd_modulT2": (12, "C"), "ayd_modulT2Yorum": (12, "D"),
        "ayd_modulO1": (13, "C"), "ayd_modulO1Yorum": (13, "D"),
        "ayd_modulO6": (16, "C"), "ayd_modulO6Yorum": (16, "D"),
        "ayd_modulO8": (17, "C"), "ayd_modulO8Yorum": (17, "D"),
        "ayd_modulI2": (20, "C"), "ayd_modulI2Yorum": (20, "D"),
    }
...
def main(cikti_yolu=None):
    ...
    varsayilan = os.path.join(kok, "Amortisman2026YenidenDegerleme.xlsx")
...
if __name__ == "__main__":
    ...
    cikti = os.path.join(repo, "cikti", "Amortisman2026YenidenDegerleme.xlsx")
