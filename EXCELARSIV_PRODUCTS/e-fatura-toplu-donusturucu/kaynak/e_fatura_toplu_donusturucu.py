#!/usr/bin/env python3
"""e-Fatura Toplu Dönüştürücü — A2 üretim betiği (manda v6, Ç15, Hat D D-02)."""
...
URUN_AD = "e-Fatura Toplu Dönüştürücü"
SURUM = "1.0.0"
RENK = "0B1F3A"
KAPASITE = 1000
DEMO = 40
...
def kapak(ws):
    ...
    h(ws, 4, 1,
      "Toplu e-Fatura/XML export satırlarını muhasebe kayıtlarıyla eşleştirip "
      "Excel deftere dönüştürür; dört kova + PDF kanıt üretir. "
      "XML dosyası okunmaz; export satırı yapıştırılır. API ve makro yoktur.",
      kaydir=True, yazi=GRİ)
    ws.merge_cells("A4:L4")
    h(ws, 6, 1, "Bu ürün ne sağlar?", kalin=True, boyut=12, yazi=KOYU_LACIVERT)
    for i, m in enumerate([
        "10.000 satır ölçek sözleşmesi + UUID normalize + fatura numarası ikincil anahtar",
...
def veri_a(ws):
    ...
    alt_bant(ws, 3, "e-Fatura/XML export satır yapıştır — XML dosyası okunmaz (API/makro yok)", son_kolon=12)
...
def motor(ws):
    ...
    adimlar = [
        ...
        (24, "Yuzdelik P90", '=IFERROR(PERCENTILE(tblEslesme[AbsFark],etd_yuzdelikOran),0)'),
        (25, "Tahmin aralik", '=IFERROR(PERCENTILE(tblVeriA[Toplam],etd_yuzdelikOran),0)'),
        (26, "Tornado zirve", '=IFERROR(MAX(PANO!K25:K29),0)'),
        (27, "Veri hazirlik",
         '=IF(C6=0,0,C6/(C6+C7))'),
        (28, "Esik alti mi", '=IF(C18<etd_esikEslesme,1,0)'),
        (29, "Fark esik ustu", '=IF(C11>etd_esikFark,1,0)'),
        (30, "Negatif A", '=COUNTIF(tblVeriA[Toplam],"<0")'),
        (31, "Negatif B", '=COUNTIF(tblVeriB[Tutar],"<0")'),
        (32, "Azami asimi A", '=COUNTIF(tblVeriA[Toplam],">"&etd_azamiTutar)'),
        (33, "Azami asimi B", '=COUNTIF(tblVeriB[Tutar],">"&etd_azamiTutar)'),
        (34, "Ikincil eslesme",
        ...
        (38, "Karar kodu",
         ...
         'IF(C18<etd_esikEslesme,"DURDUR",'
         'IF(C14>0,"İNCELE","UYGUN"))))'),
        (39, "VKN yogunluk",
         '=IFERROR(IF(C6=0,0,MAX(COUNTIF(tblVeriA[VKN],tblVeriA[VKN]))/C6),0)'),
        (40, "Senaryo etki",
         '=IFERROR(etd_toleransSenaryo*C14,0)'),
        (41, "Duyarlilik tolerans",
         '=IFERROR(ABS(C10)*etd_duyarlilikCarpan,0)'),
        (42, "Guven skoru",
         '=IF(C6=0,0,ROUND(100*C18*(1-MIN(C22,1)/etd_guvenBolum),0))'),
        (43, "Risk skoru",
         '=IF(C6=0,0,ROUND(100*(1-C18)+etd_riskFarkAgirlik*C14+etd_riskEslesmezAgirlik*C15,0))'),
        (44, "Kok neden cesit",
    ...
    duzelt = {
        ...
        33: '=IF(C18<etd_esikEslesme,1,0)',
        34: '=IF(C11>etd_esikFark,1,0)',
        ...
        45: '=IFERROR(etd_toleransSenaryo*C14,0)',
        46: '=IFERROR(ABS(C10)*etd_duyarlilikCarpan,0)',
        47: '=IF(C6=0,0,ROUND(100*C18*(1-MIN(C22,1)/etd_guvenBolum),0))',
        48: '=IF(C6=0,0,ROUND(100*(1-C18)+etd_riskFarkAgirlik*C14+etd_riskEslesmezAgirlik*C15,0))',
        50: (
            '=IFERROR(_xlfn.TEXTJOIN(" | ",TRUE,"oran",TEXT(C18,"0.0%"),'
            '"fark",TEXT(C10,"0.00")),"-")'
        ),
...
def eslestirme(ws):
    ...
    formuller = {
        ...
        "EsikUstu": (
            'IF(tblEslesme[[#This Row],[A_Norm]]="","",'
            'IF(tblEslesme[[#This Row],[AbsFark]]>etd_esikFark,1,0))'
        ),
        "NetKatki": (
            'IF(tblEslesme[[#This Row],[A_Norm]]="","",'
...
def kok_neden(ws):
    ...
    form = {
        ...
        "YuvarlamaBayrak": (
            'IF(tblKok[[#This Row],[A_Norm]]="","",'
            'IF(tblKok[[#This Row],[AbsFark]]<=etd_yuvarlamaEsik,1,0))'
        ),
        "VKN": (
            'IF(tblKok[[#This Row],[A_Norm]]="","",'
        ...
        "KokNeden": (
            ...
            'IF(ABS(tblKok[[#This Row],[AbsFark]]-tblKok[[#This Row],[Matrah]]*etd_kdvOran)<=toleransKurus,"MATRAH",'
...
def pano(ws):
    ...
    kpi = [
        ...
        (14, "Kova Bütünlük", "=etd_kovaButunluk", None),
        (15, "Anomali Sayısı", "=etd_modulO1", CATI),
        (16, "Ort. Tutar Farkı", "=etd_modulT2", TL),
        (17, "Fark P90", "=etd_modulI2", TL),
        (18, "Tahmin Aralık", "=etd_modulI1", TL),
        (19, "VKN Yoğunlaşma", "=etd_modulO6", YÜZDE),
        (20, "Veri Kalite", "=etd_modulO8", CATI),
...
def aksiyon(ws):
    ...
    form = {
        ...
        "Oncelik": (
            ...
            'IF(ABS(tblAksiyon[[#This Row],[Fark]])>etd_esikFark,"YUKSEK","NORMAL"))'
        ),
        "ItirazMetni": (
            'IF(tblAksiyon[[#This Row],[Kova]]<>"tutar_farki","",'
...
def kanit_raporu(ws):
    ...
    ozet = [
        (9, "Eşleşme Oranı", "=etd_eslesmeOrani", YÜZDE),
        (10, "e-Fatura Toplam", "=SUM(tblVeriA[Toplam])", TL),
        (11, "Mutabakat Farkı", "=etd_mutabakatFarki", TL),
        ...
        (15, "Kova Bütünlük", "=etd_kovaButunluk", None),
        (16, "Kanıt Özeti", "=etd_kanitRaporu", None),
    ]
    ...
    h(ws, 19, 1,
      ...
      "XML dosyası okunmaz; export satırı yapıştırılır. API/makro yoktur. Karar destektir.",
      kaydir=True)
...
def ornek_veri(ws):
    ...
    h(ws, 4, 1,
      f"Dosyada {DEMO} satır örnek dönüşüm vardır: eşleşen, tolerans içi, tutar farkı "
      "ve eşleşmeyen kovalar kasıtlı üretilmiştir. Ölçek hedefi 10.000 satırdır. "
      "XML dosyası okunmaz; export satırı yapıştırılır. Tireli/tiresiz UUID: satır 3, 7, 11.",
      kaydir=True)
...
def ayarlar(ws):
    ...
    params = [
        ...
        ("etd_esikEslesme", 0.90, "oran", "İç politika", "01.01.2026", "Eşleşme oranı eşiği"),
        ("etd_esikFark", 100, "TL", "İç politika", "01.01.2026", "Fark tutarı eşiği"),
        ("etd_azamiTutar", 1000000, "TL", "İç politika — uç değer", "01.01.2026", "Azami makul tutar"),
        ("etd_ikincilUzunluk", 12, "karakter", "Anahtar stratejisi SPEC", "01.01.2026", "Ikincil anahtar uzunluğu"),
        ("etd_olcekHedef", 10000, "satir", "Manda A2 Ö1", "01.01.2026", "Ölçek sözleşmesi"),
        ("etd_anomaliZ", 2.0, "z", "İstatistik kuralı", "01.01.2026", "Anomali Z eşiği"),
        ("etd_yuzdelikOran", 0.9, "oran", "İstatistik kuralı", "01.01.2026", "PERCENTILE oranı"),
        ("etd_kdvOran", 0.20, "oran", "KDV genel oran", "01.01.2026", "Varsayılan KDV"),
        ("etd_duyarlilikCarpan", 1.1, "carpan", "Duyarlılık senaryosu", "01.01.2026", "O2 çarpan"),
        ("etd_toleransSenaryo", 0.5, "TL", "Senaryo etkisi", "01.01.2026", "O3 senaryo"),
        ("etd_yuvarlamaEsik", 1, "TL", "Yuvarlama kök neden eşiği", "01.01.2026", "Abs fark ≤ eşik"),
        ("etd_guvenBolum", 10, "oran", "Güven skoru böleni", "01.01.2026", "MOTOR güven"),
        ("etd_riskFarkAgirlik", 10, "puan", "Risk — tutar farkı ağırlığı", "01.01.2026", "MOTOR risk"),
        ("etd_riskEslesmezAgirlik", 5, "puan", "Risk — eşleşmedi ağırlığı", "01.01.2026", "MOTOR risk"),
        ("etd_normAnahtar", "UPPER TRIM SUBSTITUTE tire", "metin", "Ç7 UUID anahtar", "01.01.2026", "Serbest alternatif"),
        ("etd_tanimsizKova", "TANIMSIZ görünür", "metin", "E04", "01.01.2026", "Serbest alternatif"),
        ("etd_kvkkBeyani", "Veri cihazda; XML dosyası okunmaz; API yok", "metin", "KVKK beyanı", "01.01.2026", "R3 ayrım"),
        ("etd_kanitRaporu", "A4 baskı KANIT_RAPORU", "metin", "Kanıt çıktısı", "01.01.2026", "R2 ayrım"),
        ("etd_dosyaSurumu", SURUM, "metin", "Sürüm yönetimi", "01.01.2026", "Ürün sürümü"),
        ("etd_firmaUnvan", "Örnek Ticaret A.Ş.", "metin", "Kullanıcı girişi", "01.01.2026", "Firma"),
        ("etd_fiyatModeli", "Tek seferlik alım — aylık ücret yok", "metin", "Ürün konumlandırma", "01.01.2026", "R3 ayrım"),
        ("etd_yapistirCalistir", "Kurulum beklemez; yapıştır-çalıştır", "metin", "UX vaadi", "01.01.2026", "R3 ayrım"),
    ]
    ...
    motor_cikti = [
        ...
        (35, "etd_modulO1", "=COUNTIF(tblEslesme[EsikUstu],1)"),
        (36, "etd_modulO2", "=IFERROR(ABS(etd_mutabakatFarki)*etd_duyarlilikCarpan,0)"),
        (37, "etd_modulO3", "=IFERROR(etd_toleransSenaryo*COUNTIF(tblEslesme[Kova],\"tutar_farki\"),0)"),
        (38, "etd_modulO6",
        ...
        (40, "etd_modulI1", "=IFERROR(PERCENTILE(tblVeriA[Toplam],etd_yuzdelikOran),0)"),
        (41, "etd_modulI2", "=IFERROR(PERCENTILE(tblEslesme[AbsFark],etd_yuzdelikOran),0)"),
    ]
    ...
    yorum_satir = [
        (44, "etd_modulT1Yorum",
         ...
         '_xlfn.TEXTJOIN(" ",TRUE,"Eşleşme oranı",TEXT(etd_modulT1,"0.0%")))'),
        (45, "etd_modulT2Yorum",
         '=IFERROR(_xlfn.TEXTJOIN(" ",TRUE,"Ort. tutar farkı",TEXT(etd_modulT2,"0.00"),"TL"),"-")'),
        (46, "etd_modulO1Yorum",
         '=IF(etd_modulO1=0,"Anomali yok",'
         '_xlfn.TEXTJOIN(" ",TRUE,etd_modulO1,"anomali satırı"))'),
        (47, "etd_modulO2Yorum",
         '=IFERROR(_xlfn.TEXTJOIN(" ",TRUE,"Duyarlılık etkisi",TEXT(etd_modulO2,"0.00"),"TL"),"-")'),
        (48, "etd_modulO3Yorum",
         '=IFERROR(_xlfn.TEXTJOIN(" ",TRUE,"Senaryo etki",TEXT(etd_modulO3,"0.00"),"TL"),"-")'),
        (49, "etd_modulO6Yorum",
         '=IFERROR(_xlfn.TEXTJOIN(" ",TRUE,"VKN doluluk",TEXT(etd_modulO6,"0.0%")),"-")'),
        (50, "etd_modulO8Yorum",
         '=IFERROR(_xlfn.TEXTJOIN(" ",TRUE,"Veri kalite",TEXT(etd_modulO8,"0")),"-")'),
        (51, "etd_modulI1Yorum",
         '=IFERROR(_xlfn.TEXTJOIN(" ",TRUE,"Tahmin aralık",TEXT(etd_modulI1,"0.00"),"TL"),"-")'),
        (52, "etd_modulI2Yorum",
         '=IFERROR(_xlfn.TEXTJOIN(" ",TRUE,"Fark P90",TEXT(etd_modulI2,"0.00"),"TL"),"-")'),
    ]
...
def kilavuz(ws):
    ...
    adimlar = [
        "KRİTİK: XML dosyası okunmaz; export satırı yapıştırılır (XML/API/makro yok).",
        "VERI_A: e-Fatura/XML export satırlarını (UUID, fatura numarası, tutar) sarı hücrelere yapıştırın.",
        "VERI_B: Muhasebe satırlarını girin — dosya okuma veya API gerekmez.",
    ...
    for i, m in enumerate([
        "XML dosyasını doğrudan açmaya çalışmak (desteklenmez; export satırı yapıştırın)",
...
def adlari_bagla(wb):
    for i, ana in enumerate([
        "toleransKurus", "raporTarihi", "etd_esikEslesme", "etd_esikFark",
        "etd_azamiTutar", "etd_ikincilUzunluk", "etd_olcekHedef", "etd_anomaliZ",
        "etd_yuzdelikOran", "etd_kdvOran", "etd_duyarlilikCarpan", "etd_toleransSenaryo",
        "etd_yuvarlamaEsik", "etd_guvenBolum", "etd_riskFarkAgirlik", "etd_riskEslesmezAgirlik",
        "etd_normAnahtar", "etd_tanimsizKova", "etd_kvkkBeyani", "etd_kanitRaporu",
        "etd_dosyaSurumu", "etd_firmaUnvan", "etd_fiyatModeli", "etd_yapistirCalistir",
    ]):
        ad_ekle(wb, ana, f"AYARLAR!$B${6 + i}")

    motor_map = {
        "etd_mutabakatFarki": 29, "etd_eslesmeOrani": 30, "etd_kovaButunluk": 31,
        "etd_kararKapi": 32, "etd_modulT1": 33, "etd_modulT2": 34,
        "etd_modulO1": 35, "etd_modulO2": 36, "etd_modulO3": 37,
        "etd_modulO6": 38, "etd_modulO8": 39, "etd_modulI1": 40, "etd_modulI2": 41,
    }
    for ad, r in motor_map.items():
        ad_ekle(wb, ad, f"AYARLAR!$I${r}")

    yorum_map = {
        "etd_modulT1Yorum": 44, "etd_modulT2Yorum": 45, "etd_modulO1Yorum": 46,
        "etd_modulO2Yorum": 47, "etd_modulO3Yorum": 48, "etd_modulO6Yorum": 49,
        "etd_modulO8Yorum": 50, "etd_modulI1Yorum": 51, "etd_modulI2Yorum": 52,
    }
...
def main(cikti_yolu=None):
    ...
    dosya_ad = "EFaturaTopluDonusturucu.xlsx"
