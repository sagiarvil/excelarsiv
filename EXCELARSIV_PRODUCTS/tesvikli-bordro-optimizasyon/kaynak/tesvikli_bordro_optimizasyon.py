#!/usr/bin/env python3
"""
Teşvikli Bordro Optimizasyon — üretim betiği (A1 EXCLUSIVE / manda v6).
Ayırt edici: kişi×teşvik kodu MATRIS + derin senaryo/M-SEN; Hat C C-03 giriş SKU değil.
"""
...
URUN_AD = "Teşvikli Bordro Optimizasyon"
SURUM = "1.0.0"
RENK = "0F2742"
...
def girdi(ws):
    ...
    h(ws, 18, 2, "=IFERROR(tbo_potansiyelTasarruf,0)", sayi=TL, yazi=DIS_REF_YEŞIL, kalin=True)
    h(ws, 19, 1, "Giriş doluluk (kalite)", kalin=True, yazi=KOYU_LACIVERT)
    h(ws, 19, 2,
      '=IFERROR(IF(OR(COUNTA(B6:B16)=0,tbo_girisBeklenen=0),0,ROUND(COUNTA(B6:B16)/tbo_girisBeklenen*100,0)),0)',
...
def bordro(ws):
    """Çalışan satır bazlı bordro — kişi başı en avantajlı teşvik kodu."""
    sayfa_hazirla(ws, "BORDRO", "C8A96A", URUN_AD, son_kolon=40)
    h(ws, 3, 1, "Çalışan satır bordrosu — kişi başı en avantajlı teşvik",
      kalin=True, boyut=13, yazi=KOYU_LACIVERT, kaydir=True)
    ws.merge_cells("A3:N3")
    h(ws, 4, 1,
      "Sarı hücreler manuel giriş. En avantajlı kod ve tasarruf formülle üretilir "
      "(oranlar KURALLAR tablosundan; sabit oran yok).",
      yazi=GRİ, kaydir=True)
    ws.merge_cells("A4:N4")


    sutunlar = [
        "CalisanNo", "AdSoyad", "BrutMatrah", "BesPuanUygun", "IlaveUygun",
        "KadinGencUygun", "EngelliUygun", "MevcutKod",
        "EnAvantajliKod", "EnAvantajliTutar", "MevcutTutar", "Kacirilan",
    ]
    baslik_satiri(ws, 6, sutunlar)

    # Oranlar ad tanımından; satırda INDEX/MATCH yok (Ö1)
    formuller = {
        "EnAvantajliTutar": (
            '=IF(tblBordro[[#This Row],[CalisanNo]]="","",'
            'MAX('
            'IF(tblBordro[[#This Row],[BesPuanUygun]]="Evet",ROUND(tblBordro[[#This Row],[BrutMatrah]]*tbo_oranBesPuan,2),0),'
            'IF(tblBordro[[#This Row],[IlaveUygun]]="Evet",ROUND(tblBordro[[#This Row],[BrutMatrah]]*tbo_oranIlave,2),0),'
            'IF(tblBordro[[#This Row],[KadinGencUygun]]="Evet",ROUND(tblBordro[[#This Row],[BrutMatrah]]*tbo_oranKadinGenc,2),0),'
            'IF(tblBordro[[#This Row],[EngelliUygun]]="Evet",ROUND(tblBordro[[#This Row],[BrutMatrah]]*tbo_isverenOran,2),0)))'
        ),
        "EnAvantajliKod": (
            '=IF(tblBordro[[#This Row],[CalisanNo]]="","",'
            'IF(tblBordro[[#This Row],[EnAvantajliTutar]]=0,"YOK",'
            'IF(AND(tblBordro[[#This Row],[IlaveUygun]]="Evet",'
            'ROUND(tblBordro[[#This Row],[BrutMatrah]]*tbo_oranIlave,2)=tblBordro[[#This Row],[EnAvantajliTutar]]),"TBO-002",'
            'IF(AND(tblBordro[[#This Row],[BesPuanUygun]]="Evet",'
            'ROUND(tblBordro[[#This Row],[BrutMatrah]]*tbo_oranBesPuan,2)=tblBordro[[#This Row],[EnAvantajliTutar]]),"TBO-001",'
            'IF(AND(tblBordro[[#This Row],[KadinGencUygun]]="Evet",'
            'ROUND(tblBordro[[#This Row],[BrutMatrah]]*tbo_oranKadinGenc,2)=tblBordro[[#This Row],[EnAvantajliTutar]]),"TBO-004","TBO-ENG")))))'
        ),
        "MevcutTutar": (
            '=IF(tblBordro[[#This Row],[CalisanNo]]="","",'
            'IF(tblBordro[[#This Row],[MevcutKod]]="TBO-001",'
            'IF(tblBordro[[#This Row],[BesPuanUygun]]="Evet",ROUND(tblBordro[[#This Row],[BrutMatrah]]*tbo_oranBesPuan,2),0),'
            'IF(tblBordro[[#This Row],[MevcutKod]]="TBO-002",'
            'IF(tblBordro[[#This Row],[IlaveUygun]]="Evet",ROUND(tblBordro[[#This Row],[BrutMatrah]]*tbo_oranIlave,2),0),'
            'IF(tblBordro[[#This Row],[MevcutKod]]="TBO-004",'
            'IF(tblBordro[[#This Row],[KadinGencUygun]]="Evet",ROUND(tblBordro[[#This Row],[BrutMatrah]]*tbo_oranKadinGenc,2),0),'
            'IF(tblBordro[[#This Row],[MevcutKod]]="TBO-ENG",'
            'IF(tblBordro[[#This Row],[EngelliUygun]]="Evet",ROUND(tblBordro[[#This Row],[BrutMatrah]]*tbo_isverenOran,2),0),0)))))'
        ),
        "Kacirilan": (
            '=IF(tblBordro[[#This Row],[CalisanNo]]="","",'
            'MAX(0,tblBordro[[#This Row],[EnAvantajliTutar]]-tblBordro[[#This Row],[MevcutTutar]]))'
        ),
    }
    # Kapasite 1000 satır (başlık 6 → veri 7..1006)
    tablo_ekle(ws, "tblBordro", "A6:L1006", sutunlar, formuller)

    demo = [
        (1, "Ali Yılmaz", 25000, "Evet", "Hayır", "Hayır", "Hayır", "TBO-001"),
        (2, "Ayşe Demir", 22000, "Evet", "Evet", "Hayır", "Hayır", "TBO-001"),
        (3, "Mehmet Kaya", 28000, "Evet", "Hayır", "Evet", "Hayır", "TBO-004"),
        (4, "Zeynep Arslan", 20000, "Hayır", "Evet", "Hayır", "Hayır", "YOK"),
        (5, "Can Öztürk", 30000, "Evet", "Evet", "Hayır", "Evet", "TBO-001"),
        (6, "Elif Şahin", 18000, "Evet", "Hayır", "Evet", "Hayır", "TBO-001"),
        (7, "Burak Çelik", 35000, "Evet", "Evet", "Hayır", "Hayır", "TBO-002"),
        (8, "Selin Aydın", 24000, "Hayır", "Hayır", "Evet", "Hayır", "YOK"),
        (9, "Emre Koç", 27000, "Evet", "Hayır", "Hayır", "Evet", "TBO-001"),
        (10, "Deniz Yıldız", 21000, "Evet", "Evet", "Evet", "Hayır", "TBO-001"),
    ]
    for i, row in enumerate(demo, 7):
        for k, v in enumerate(row, 1):
            ...
            for kolon in range(1, 9):
                ws.cell(i, kolon).fill = PatternFill(
                    "solid", start_color=GIRIS_SARI, end_color=GIRIS_SARI)
                ws.cell(i, kolon).protection = Protection(locked=False)
                _kim(ws, f"{get_column_letter(kolon)}{i}", sutunlar[kolon - 1],
                     "Bordro satır girişi; uygunluk Evet/Hayır")

    # Özet paneli
    h(ws, 3, 18, "Özet", kalin=True, yazi="FFFFFF", zemin=KOYU_LACIVERT)
    h(ws, 4, 18, "Çalışan adedi", yazi="333333")
    h(ws, 4, 19, '=IFERROR(COUNTA(tblBordro[CalisanNo]),0)', sayi=CATI, yazi=DIS_REF_YEŞIL)
    h(ws, 5, 18, "Toplam brüt matrah", yazi="333333")
    h(ws, 5, 19, '=IFERROR(SUM(tblBordro[BrutMatrah]),0)', sayi=TL, yazi=DIS_REF_YEŞIL)
    h(ws, 6, 18, "En avantajlı toplam", yazi="333333")
    h(ws, 6, 19, '=IFERROR(SUM(tblBordro[EnAvantajliTutar]),0)', sayi=TL, yazi=DIS_REF_YEŞIL)
    h(ws, 7, 18, "Mevcut toplam", yazi="333333")
    h(ws, 7, 19, '=IFERROR(SUM(tblBordro[MevcutTutar]),0)', sayi=TL, yazi=DIS_REF_YEŞIL)
    h(ws, 8, 18, "Kaçırılan toplam", yazi="333333")
    h(ws, 8, 19, '=IFERROR(SUM(tblBordro[Kacirilan]),0)', sayi=TL, yazi=DIS_REF_YEŞIL, kalin=True)
    h(ws, 9, 18, "En sık avantajlı kod", yazi="333333")
    h(ws, 9, 19,
      '=IFERROR(IF(COUNTA(tblBordro[CalisanNo])=0,"VERİ YOK",'
      'IF(COUNTIF(tblBordro[EnAvantajliKod],"TBO-002")>=MAX('
      'COUNTIF(tblBordro[EnAvantajliKod],"TBO-001"),'
      'COUNTIF(tblBordro[EnAvantajliKod],"TBO-002"),'
      'COUNTIF(tblBordro[EnAvantajliKod],"TBO-004")),"TBO-002",'
      'IF(COUNTIF(tblBordro[EnAvantajliKod],"TBO-001")>=COUNTIF(tblBordro[EnAvantajliKod],"TBO-004"),'
      '"TBO-001","TBO-004"))),"-")',
      yazi=DIS_REF_YEŞIL, kalin=True)

    # Doğrulama — tablo sınırıyla uyumlu (satır 1006)
    dogrulama(ws, "list", "ListeEvetHayir", "D7:G1006",
              baslik="Uygunluk", mesaj="Evet veya Hayır seçin.",
              hata_baslik="Geçersiz", hata_mesaj="Listeden seçin.")
    dogrulama(ws, "list", "ListeTesvikKod", "H7:H1006",
              baslik="Mevcut kod", mesaj="Uygulanan teşvik kodunu seçin.",
              hata_baslik="Geçersiz", hata_mesaj="Listeden seçin.")
    dogrulama(ws, "decimal", "0", "C7:C1006",
              baslik="Brüt matrah", mesaj="Kişi brüt SGK matrahını TL girin.",
              hata_baslik="Geçersiz", hata_mesaj="0 ile 1e12 arasında.",
              isaret="between", f2="1000000000000")
    dogrulama(ws, "whole", "1", "A7:A1006",
              baslik="Çalışan no", mesaj="Benzersiz çalışan numarası.",
              hata_baslik="Geçersiz", hata_mesaj="1 ve üzeri tam sayı.",
              isaret="between", f2="100000")

    giris_hucreleri(ws, 7, 16, [1, 2, 3, 4, 5, 6, 7, 8])
    # boş kapasite satırları için sarı + yorum (ilk 50 ekstra satır yeterli prompt için;
    # tablo_formullerini_hucrelere_yaz sonrası kalanlar formül alır)
    for i in range(17, 57):
        for kolon in range(1, 9):
            cell = ws.cell(i, kolon)
            if cell.value in (None, ""):
                cell.fill = PatternFill("solid", start_color=GIRIS_SARI, end_color=GIRIS_SARI)
                cell.protection = Protection(locked=False)
                _kim(ws, f"{get_column_letter(kolon)}{i}", sutunlar[kolon - 1],
                     "Yeni çalışan satırı; boş bırakılabilir")

    # Grafik demo sabitleri
    h(ws, 12, 18, "Kod", kalin=True, yazi="FFFFFF", zemin=ORTA_MAVI)
    h(ws, 12, 19, "AdetDemo", kalin=True, yazi="FFFFFF", zemin=ORTA_MAVI)
    for i, (kod, adet) in enumerate([
        ("TBO-001", 4), ("TBO-002", 3), ("TBO-004", 2), ("YOK", 1),
    ], 13):
        h(ws, i, 18, kod, yazi=GRİ)
        h(ws, i, 19, adet, sayi=CATI, yazi=GRİ)
    g = BarChart()
    g.type = "col"
    g.title = "En Avantajlı Kod Dağılımı"
    g.add_data(Reference(ws, min_col=19, min_row=12, max_row=16), titles_from_data=True)
    g.set_categories(Reference(ws, min_col=18, min_row=13, max_row=16))
    g.height, g.width = 8, 12
    ws.add_chart(g, "R18")

    sabitle(ws, "A7")
    genislik(ws, {get_column_letter(i): w for i, w in enumerate(
        [12, 18, 14, 14, 12, 14, 14, 12, 14, 14, 12, 12], 1)})
    genislik(ws, {"R": 22, "S": 14})
    alt_bant(ws, 1008, "BORDRO: sarı = giriş; EnAvantajliKod formülle seçilir.")


def matris(ws):
    """Kişi × teşvik kodu optimizasyon matrisi — Hat D EXCLUSIVE ayırt edici."""
    sayfa_hazirla(ws, "MATRIS", "8B5A2B", URUN_AD, son_kolon=40)
    h(ws, 3, 1, "Kişi × teşvik kodu optimizasyon matrisi",
      kalin=True, boyut=13, yazi=KOYU_LACIVERT, kaydir=True)
    ws.merge_cells("A3:L3")
    h(ws, 4, 1,
      "Sarı hücreler giriş. Her kod kolonu o kişi için o teşvikin tutarını üretir; "
      "EnIyiKod satır bazında MAX ile seçilir (INDEX/MATCH yok — ölçek dostu).",
      yazi=GRİ, kaydir=True)
    ws.merge_cells("A4:L4")

    sutunlar = [
        "CalisanNo", "AdSoyad", "BrutMatrah",
        "BesPuanUygun", "IlaveUygun", "KadinGencUygun", "EngelliUygun",
        "TBO001", "TBO002", "TBO004", "TBOENG",
        "EnIyiKod", "EnIyiTutar", "MevcutKod", "Fark",
    ]
    baslik_satiri(ws, 6, sutunlar)

    formuller = {
        "TBO001": (
            '=IF(tblMatris[[#This Row],[CalisanNo]]="","",'
            'IF(tblMatris[[#This Row],[BesPuanUygun]]="Evet",'
            'ROUND(tblMatris[[#This Row],[BrutMatrah]]*tbo_oranBesPuan,2),0))'
        ),
        "TBO002": (
            '=IF(tblMatris[[#This Row],[CalisanNo]]="","",'
            'IF(tblMatris[[#This Row],[IlaveUygun]]="Evet",'
            'ROUND(tblMatris[[#This Row],[BrutMatrah]]*tbo_oranIlave,2),0))'
        ),
        "TBO004": (
            '=IF(tblMatris[[#This Row],[CalisanNo]]="","",'
            'IF(tblMatris[[#This Row],[KadinGencUygun]]="Evet",'
            'ROUND(tblMatris[[#This Row],[BrutMatrah]]*tbo_oranKadinGenc,2),0))'
        ),
        "TBOENG": (
            '=IF(tblMatris[[#This Row],[CalisanNo]]="","",'
            'IF(tblMatris[[#This Row],[EngelliUygun]]="Evet",'
            'ROUND(tblMatris[[#This Row],[BrutMatrah]]*tbo_isverenOran,2),0))'
        ),
        "EnIyiTutar": (
            '=IF(tblMatris[[#This Row],[CalisanNo]]="","",'
            'MAX(tblMatris[[#This Row],[TBO001]],tblMatris[[#This Row],[TBO002]],'
            'tblMatris[[#This Row],[TBO004]],tblMatris[[#This Row],[TBOENG]]))'
        ),
        "EnIyiKod": (
            '=IF(tblMatris[[#This Row],[CalisanNo]]="","",'
            'IF(tblMatris[[#This Row],[EnIyiTutar]]=0,"YOK",'
            'IF(tblMatris[[#This Row],[TBO002]]=tblMatris[[#This Row],[EnIyiTutar]],"TBO-002",'
            'IF(tblMatris[[#This Row],[TBO001]]=tblMatris[[#This Row],[EnIyiTutar]],"TBO-001",'
            'IF(tblMatris[[#This Row],[TBO004]]=tblMatris[[#This Row],[EnIyiTutar]],"TBO-004","TBO-ENG")))))'
        ),
        "Fark": (
            '=IF(tblMatris[[#This Row],[CalisanNo]]="","",'
            'MAX(0,tblMatris[[#This Row],[EnIyiTutar]]-'
            'IF(tblMatris[[#This Row],[MevcutKod]]="TBO-001",tblMatris[[#This Row],[TBO001]],'
            'IF(tblMatris[[#This Row],[MevcutKod]]="TBO-002",tblMatris[[#This Row],[TBO002]],'
            'IF(tblMatris[[#This Row],[MevcutKod]]="TBO-004",tblMatris[[#This Row],[TBO004]],'
            'IF(tblMatris[[#This Row],[MevcutKod]]="TBO-ENG",tblMatris[[#This Row],[TBOENG]],0))))))'
        ),
    }
    tablo_ekle(ws, "tblMatris", "A6:O1006", sutunlar, formuller)
    ...
    # Giriş kolonları: A-G + N (MevcutKod = kol 14)
    giris_kolonlari = [1, 2, 3, 4, 5, 6, 7, 14]
    for i, row in enumerate(demo, 7):
        for k, v in enumerate(row[:7], 1):
            ws.cell(i, k).value = v
            if k == 1:
                ws.cell(i, k).number_format = CATI
            elif k == 3:
                ws.cell(i, k).number_format = TL
        ws.cell(i, 14).value = row[7]
        for kolon in giris_kolonlari:
            ws.cell(i, kolon).fill = PatternFill(
                "solid", start_color=GIRIS_SARI, end_color=GIRIS_SARI)
            ws.cell(i, kolon).protection = Protection(locked=False)
            _kim(ws, f"{get_column_letter(kolon)}{i}", sutunlar[kolon - 1],
                 "Matris satır girişi; BORDRO ile tutarlı tutun")
    ...
    dogrulama(ws, "list", "ListeEvetHayir", "D7:G1006",
              baslik="Uygunluk", mesaj="Evet veya Hayır seçin.",
              hata_baslik="Geçersiz", hata_mesaj="Listeden seçin.")
    dogrulama(ws, "list", "ListeTesvikKod", "N7:N1006",
              baslik="Mevcut kod", mesaj="Uygulanan teşvik kodunu seçin.",
              hata_baslik="Geçersiz", hata_mesaj="Listeden seçin.")
    dogrulama(ws, "decimal", "0", "C7:C1006",
              baslik="Brüt matrah", mesaj="Kişi brüt SGK matrahını TL girin.",
    ...
    dogrulama(ws, "whole", "1", "A7:A1006",
              ...
              hata_baslik="Geçersiz", hata_mesaj="1 ve üzeri tam sayı.",
              isaret="between", f2="100000")
    giris_hucreleri(ws, 7, 56, giris_kolonlari)

    h(ws, 3, 17, "Matris özeti", kalin=True, yazi="FFFFFF", zemin=KOYU_LACIVERT)
    h(ws, 4, 17, "Kişi sayısı", yazi="333333")
    h(ws, 4, 18, '=IFERROR(COUNTA(tblMatris[CalisanNo]),0)', sayi=CATI, yazi=DIS_REF_YEŞIL)
    h(ws, 5, 17, "En iyi set toplamı", yazi="333333")
    h(ws, 5, 18, '=IFERROR(SUM(tblMatris[EnIyiTutar]),0)', sayi=TL, yazi=DIS_REF_YEŞIL, kalin=True)
    h(ws, 6, 17, "Matris kaçırılan", yazi="333333")
    h(ws, 6, 18, '=IFERROR(SUM(tblMatris[Fark]),0)', sayi=TL, yazi=DIS_REF_YEŞIL, kalin=True)
    h(ws, 7, 17, "Kod seti özeti", yazi="333333")
    h(ws, 7, 18,
      '=IFERROR(IF(COUNTA(tblMatris[CalisanNo])=0,"VERİ YOK",'
      '_xlfn.TEXTJOIN(" · ",TRUE,'
      '"TBO-001=",COUNTIF(tblMatris[EnIyiKod],"TBO-001"),'
      '"TBO-002=",COUNTIF(tblMatris[EnIyiKod],"TBO-002"),'
      '"TBO-004=",COUNTIF(tblMatris[EnIyiKod],"TBO-004"),'
      '"TBO-ENG=",COUNTIF(tblMatris[EnIyiKod],"TBO-ENG"),'
      '"YOK=",COUNTIF(tblMatris[EnIyiKod],"YOK"))),"-")',
      yazi=DIS_REF_YEŞIL, kaydir=True)
    ws.merge_cells("R7:W7")
    h(ws, 8, 17, "Optimizasyon kararı", yazi="333333")
    h(ws, 8, 18,
      '=IFERROR(IF(COUNTA(tblMatris[CalisanNo])=0,"VERİ YOK",'
      'IF(R6=0,"UYGUN",IF(R6>=tbo_kacirilanEsik,"UYGULA","İNCELE"))),"-")',
      yazi=DIS_REF_YEŞIL, kalin=True)

    h(ws, 10, 17, "Kod", kalin=True, yazi="FFFFFF", zemin=ORTA_MAVI)
    h(ws, 10, 18, "SetAdetDemo", kalin=True, yazi="FFFFFF", zemin=ORTA_MAVI)
    for i, (kod, adet) in enumerate([
        ("TBO-001", 4), ("TBO-002", 3), ("TBO-004", 2), ("TBO-ENG", 1),
    ], 11):
        h(ws, i, 17, kod, yazi=GRİ)
        h(ws, i, 18, adet, sayi=CATI, yazi=GRİ)
    g = BarChart()
    g.type = "col"
    g.title = "Matris En İyi Kod Seti"
    g.add_data(Reference(ws, min_col=18, min_row=10, max_row=14), titles_from_data=True)
    g.set_categories(Reference(ws, min_col=17, min_row=11, max_row=14))
    g.height, g.width = 8, 12
    ws.add_chart(g, "Q16")

    sabitle(ws, "A7")
    genislik(ws, {get_column_letter(i): w for i, w in enumerate(
        [12, 18, 14, 14, 12, 14, 14, 12, 12, 12, 12, 12, 12, 12, 12], 1)})
    genislik(ws, {"Q": 22, "R": 55})
    alt_bant(ws, 1008, "MATRIS: sarı = giriş; kod tutarları formülle üretilir.")
...
def kurallar(ws):
    ...
    satirlar = [
        ["TBO-001", "5510 md.81", "bes_puan_oran", "kapsam_matrah>0", "bes_puan_oran",
         0.05, 0.05, 0.05, "01.01.2025", "5510 s.K. md.81"],
        ["TBO-002", "4447 geç.10", "ilave_istihdam_oran", "ilave_matrah>0", "ilave_istihdam_oran",
         0.145, 0.145, 0.145, "01.01.2025", "4447 s.K. geç.10"],
        ["TBO-003", "SGK tavan", "prim_tavani", "her hesap", "prim_tavani",
         150000, 165000, 180000, "01.01.2025", "SGK prim tavanı"],
        ["TBO-004", "4447 / 5510", "kadin_genc_oran", "kadin_genc_matrah>0", "kadin_genc_oran",
         0.05, 0.05, 0.05, "01.01.2025", "Kadın/genç istihdam"],
        ["TBO-005", "Genel", "yuvarlama", "her hesap", "yuvarlama_ondalik",
    ...
    for i, s in enumerate(satirlar, 7):
        for k, v in enumerate(s, 1):
            ...
            if k in (6, 7, 8) and s[0] in ("TBO-003", "TBO-005"):
                sayi = CATI if s[0] == "TBO-005" else TL
    ...
    formuller = {
        "aktif_deger": (
            ...
            "IFERROR(CHOOSE(MAX(1,MIN(3,IF(OR(hesapYili=\"\",hesapYili=0),1,hesapYili-tbo_yilTaban))),"
    ...
    h(ws, 15, 2, kural_cek("TBO-001"), sayi=YÜZDE, yazi=DIS_REF_YEŞIL)
    h(ws, 16, 1, "İlave istihdam oranı (aktif yıl)", yazi="333333")
    h(ws, 16, 2, kural_cek("TBO-002"), sayi=YÜZDE, yazi=DIS_REF_YEŞIL)
    h(ws, 17, 1, "Matris metni", yazi="333333")
    h(ws, 17, 2,
      '=IFERROR(_xlfn.TEXTJOIN(" | ",TRUE,"TBO-001=",TEXT(B15,"0.0%"),"TBO-002=",TEXT(B16,"0.0%"),'
...
def motor(ws):
    ...
    ekle("Hesap yılı oku", "=hesapYili", "yıl", "Aktif mevzuat yılı", "TBO-005")  # G7
    ekle("5 puan oranı", km("TBO-001"), "oran", "5510 md.81 oranı", "TBO-001")  # G8
    ekle("İlave istihdam oranı", km("TBO-002"), "oran", "4447 geç.10 oranı", "TBO-002")  # G9
    ekle("Prim tavanı", km("TBO-003"), "TL", "Aylık prim tavanı", "TBO-003")  # G10
    ekle("Kadın/genç oranı", km("TBO-004"), "oran", "Kadın/genç teşvik oranı", "TBO-004")  # G11
    ekle("Yuvarlama ondalık", km("TBO-005"), "adet", "Yuvarlama basamağı", "TBO-005")  # G12
    ekle("Aylık brüt matrah", '=IF(COUNTA(tblBordro[CalisanNo])>0,SUM(tblBordro[BrutMatrah]),GIRDI!B9)', "TL", "Ham brüt matrah", "TBO-003")  # G13
    ekle("Çalışan sayısı", '=IF(COUNTA(tblBordro[CalisanNo])>0,COUNTA(tblBordro[CalisanNo]),GIRDI!B10)', "adet", "Bildirilen çalışan", "TBO-003")  # G14
    ekle("5 puan kapsam", '=IF(COUNTA(tblBordro[CalisanNo])>0,SUMIF(tblBordro[BesPuanUygun],"Evet",tblBordro[BrutMatrah]),GIRDI!B11)', "TL", "5 puan uygun matrah", "TBO-001")  # G15
    ekle("İlave kapsam", '=IF(COUNTA(tblBordro[CalisanNo])>0,SUMIF(tblBordro[IlaveUygun],"Evet",tblBordro[BrutMatrah]),GIRDI!B12)', "TL", "İlave uygun matrah", "TBO-002")  # G16
    ekle("Kadın/genç kapsam", '=IF(COUNTA(tblBordro[CalisanNo])>0,SUMIF(tblBordro[KadinGencUygun],"Evet",tblBordro[BrutMatrah]),GIRDI!B13)', "TL", "Kadın/genç matrah", "TBO-004")  # G17
    ekle("Engelli kapsam", '=IF(COUNTA(tblBordro[CalisanNo])>0,SUMIF(tblBordro[EngelliUygun],"Evet",tblBordro[BrutMatrah]),GIRDI!B14)', "TL", "Engelli matrah", "TBO-002")  # G18
    ekle("Mevcut teşvik", '=IF(COUNTA(tblBordro[CalisanNo])>0,SUM(tblBordro[MevcutTutar]),GIRDI!B15)', "TL", "Fiili uygulanan", "TBO-005")  # G19
    ekle("Tavan üst sınırı", "=G10*G14", "TL", "Tavan × çalışan", "TBO-003")  # G20
    ekle("Matrah tavanlı", "=MIN(G13,G20)", "TL", "Tavan kesilmiş matrah", "TBO-003")  # G21
    ekle("5 puan (kesilmiş)", f"=ROUND(MIN(G15,G21)*G8,{_YV})", "TL", "5 puan tasarrufu", "TBO-001")  # G22
    ekle("İlave (kesilmiş)", f"=ROUND(MIN(G16,G21)*G9,{_YV})", "TL", "İlave tasarrufu", "TBO-002")  # G23
    ekle("Kadın/genç (kesilmiş)", f"=ROUND(MIN(G17,G21)*G11,{_YV})", "TL", "Kadın/genç tasarrufu", "TBO-004")  # G24
    ekle("Engelli (işveren payı)", f"=ROUND(MIN(G18,G21)*tbo_isverenOran,{_YV})", "TL",
         "Engelli işveren payı muafiyeti", "TBO-002")  # G25
    ekle("Birikimli toplam (A)", "=G22+G23+G24+G25", "TL", "Yorum A birikim", "TBO-005")  # G26
    ekle("En yüksek tek kalem (B)", "=MAX(G22,G23,G24,G25)", "TL", "Yorum B tek seçim", "TBO-005")  # G27
    ekle("Potansiyel tasarruf",
         '=IF(GIRDI!B16="B",G27,G26)',
         "TL", "Seçili yoruma göre", "TBO-005")  # G28
    ekle("Kaçırılan (ham)", "=G28-G19", "TL", "Potansiyel − mevcut", "TBO-005")  # G29
    ekle("Kaçırılan (sıfır altı yok)", "=MAX(0,G29)", "TL", "Optimizasyon fırsatı", "TBO-005")  # G30
    ekle("Yıllık potansiyel", "=G28*12", "TL", "12 ay projeksiyon", "TBO-005")  # G31
    ekle("Yıllık kaçırılan", "=G30*12", "TL", "12 ay kaçırılan", "TBO-005")  # G32
    ekle("Karar kodu",
         '=IF(COUNTA(tblBordro[CalisanNo])=0,"VERI_YOK",'
         'IF(OR(MIN(tblBordro[BrutMatrah])<0,MAX(tblBordro[BrutMatrah])>tbo_azamiMatrahEsik),"UC",'
         'IF(G30=0,"TAM","FIRSAT")))',
         "kod", "Karar kodu", "TBO-005")  # G33
    ekle("Karar metni",
         '=IF(G33="VERI_YOK","VERİ YOK",'
         'IF(G33="UC","İNCELE",'
         'IF(G33="TAM","UYGUN",'
         'IF(G30>=tbo_kacirilanEsik,"UYGULA","İNCELE"))))',
         "metin", "Kullanıcıya karar", "TBO-005")  # G34
    ekle("Gerekçe",
         '=IF(G33="VERI_YOK","Bordro satırı yok — hesap yapılamaz.",'
         'IF(G33="UC","Uç değer (negatif veya aşırı matrah) — satırları inceleyin.",'
         'IF(G33="TAM","Mevcut teşvik potansiyeli karşılıyor; ek fırsat yok.",'
         '"Kaçırılan tasarruf var; en avantajlı teşvik kodunu uygulayın veya inceleyin.")))',
         "metin", "Gerekçe cümlesi", "TBO-005")  # G35
    ekle("Kapsam / matrah oranı", "=IF(G21=0,0,(G15+G16+G17+G18)/G21)", "oran",
         "Kapsam yoğunluğu", "TBO-003")  # G36
    ekle("Mevcut / potansiyel", "=IF(G28=0,0,G19/G28)", "oran", "Karşılama oranı", "TBO-005")  # G37
    ekle("Güven skoru",
         "=IFERROR(ROUND(IF(OR(COUNTA(GIRDI!B6:B16)=0,tbo_girisBeklenen=0),0,"
         "COUNTA(GIRDI!B6:B16)/tbo_girisBeklenen*100),0),0)",
         "puan", "Giriş bütünlüğü", "TBO-005")  # G38
    ekle("Risk skoru",
         '=IF(G33="VERI_YOK",0,IF(G30=0,tbo_riskDusuk,'
         'IF(G30/MAX(G28,1)>tbo_riskEsikOran,tbo_riskYuksek,tbo_riskOrta)))',
         "puan", "Kaçırılan riski", "TBO-005")  # G39
    ekle("Senaryo iyimser matrah", "=G21*tbo_senaryoIyi", "TL", "İyimser matrah", "TBO-003")  # G40
    ekle("Senaryo kötümser matrah", "=G21*tbo_senaryoKotu", "TL", "Kötümser matrah", "TBO-003")  # G41
    ekle("İyimser potansiyel",
         f"=ROUND(MIN(G15,G40)*G8+MIN(G16,G40)*G9+MIN(G17,G40)*G11+MIN(G18,G40)*tbo_isverenOran,{_YV})",
         "TL", "İyimser birikim", "TBO-001")  # G42
    ekle("Kötümser potansiyel",
         f"=ROUND(MIN(G15,G41)*G8+MIN(G16,G41)*G9+MIN(G17,G41)*G11+MIN(G18,G41)*tbo_isverenOran,{_YV})",
         "TL", "Kötümser birikim", "TBO-001")  # G43
    ekle("Kural+1 puan 5 puan", f"=ROUND(MIN(G15,G21)*(G8+tbo_oranArtisPuan),{_YV})", "TL",
         "M-SEN +1 puan", "TBO-001")  # G44
    ekle("M-SEN potansiyel", "=G44+G23+G24+G25", "TL", "Kural değişimi toplam", "TBO-001")  # G45
    ekle("M-SEN kaçırılan", "=MAX(0,G45-G19)", "TL", "M-SEN farkı", "TBO-005")  # G46
    ekle("Tahmin üst", "=G30*tbo_tahminUst", "TL", "Tahmin üst bant", "TBO-005")  # G47
    ekle("Tahmin alt", "=G30*tbo_tahminAlt", "TL", "Tahmin alt bant", "TBO-005")  # G48
    ekle("Tornado: matrah etkisi",
         f"=ABS(ROUND(MIN(G15,G21*tbo_tornadoMatrah)*G8+MIN(G16,G21*tbo_tornadoMatrah)*G9"
         f"+MIN(G17,G21*tbo_tornadoMatrah)*G11+MIN(G18,G21*tbo_tornadoMatrah)*tbo_isverenOran,{_YV})-G28)",
         "TL", "Matrah ± etki", "TBO-003")  # G49
    ekle("Tornado: mevcut etkisi",
         "=ABS(MAX(0,G28-G19*tbo_tornadoMevcut)-G30)",
         "TL", "Mevcut ± etki", "TBO-005")  # G50
    ekle("Tornado: oran etkisi",
         f"=ABS(ROUND(MIN(G15,G21)*(G8+tbo_oranArtisPuan)+G23+G24+G25,{_YV})-G28)",
         "TL", "Oran ± etki", "TBO-001")  # G51
    ekle("Madde atıf özeti",
         '=_xlfn.TEXTJOIN(" | ",TRUE,"TBO-001→5510 md.81","TBO-002→4447 geç.10","TBO-003→prim tavanı")',
         "metin", "Kanıt atıfları", "TBO-001")  # G52
    ekle("Kanıt satır özeti",
         '=_xlfn.TEXTJOIN(" · ",TRUE,"Matrah=",TEXT(G21,"₺ #,##0"),"Potansiyel=",TEXT(G28,"₺ #,##0"),'
         '"Kaçırılan=",TEXT(G30,"₺ #,##0"))',
         "metin", "Rapor özeti", "TBO-005")  # G53
...
def senaryo(ws):
    ...
    pot = "IFERROR(tbo_potansiyelTasarruf,0)"
    mev = "IFERROR(tbo_mevcutTesvik,0)"
    karar_tpl = (
        '=IF(COUNTA(tblBordro[CalisanNo])=0,"VERİ YOK",'
        'IF(D{r}=0,"UYGUN",IF(D{r}>=tbo_kacirilanEsik,"UYGULA","İNCELE")))'
    )
    senaryolar = [
        ("İyimser", "=IFERROR(tbo_senaryoIyi+N(GIRDI!B9)*0,0)",
        ...
        ("Kötümser", "=IFERROR(tbo_senaryoKotu+N(GIRDI!B9)*0,0)",
         f"=IFERROR(ROUND(({pot})*B8,2),0)",
         f"=IFERROR(MAX(0,C8-({mev})),0)"),
        ("M-SEN +1 puan", "=IFERROR(1+N(GIRDI!B9)*0,1)",
         "=IFERROR(MOTOR!G45,0)",
         "=IFERROR(MOTOR!G46,0)"),
        ("Agresif matrah", "=IFERROR(tbo_senaryoIyi*0.95+N(GIRDI!B9)*0,0)",
         f"=IFERROR(ROUND(({pot})*B10,2),0)",
         f"=IFERROR(MAX(0,C10-({mev})),0)"),
        ("Muhafazakar", "=IFERROR(tbo_senaryoKotu*1.05+N(GIRDI!B9)*0,0)",
         f"=IFERROR(ROUND(({pot})*B11,2),0)",
         f"=IFERROR(MAX(0,C11-({mev})),0)"),
        ("Matris seti", "=IFERROR(1+N(GIRDI!B9)*0,1)",
         "=IFERROR(MATRIS!R5,0)",
         "=IFERROR(MATRIS!R6,0)"),
    ]
    for i, (ad, carp, asg, fark) in enumerate(senaryolar, 6):
        ...
        h(ws, i, 5, karar_tpl.format(r=i), yazi=DIS_REF_YEŞIL)

    h(ws, 14, 1, "Senaryo bant genişliği (iyimser−kötümser kaçırılan)", kalin=True, yazi=KOYU_LACIVERT)
    h(ws, 14, 2, "=IFERROR(D6-D8,0)", sayi=TL, yazi=DIS_REF_YEŞIL)
    h(ws, 15, 1, "Senaryo yorumu", kalin=True, yazi=KOYU_LACIVERT)
    h(ws, 15, 2,
      '=IFERROR(_xlfn.TEXTJOIN("; ",TRUE,"İyimser ",TEXT(D6,"₺ #,##0")," · Baz ",TEXT(D7,"₺ #,##0"),'
      '" · Kötümser ",TEXT(D8,"₺ #,##0")," · M-SEN ",TEXT(D9,"₺ #,##0"),'
      '" · Matris ",TEXT(D12,"₺ #,##0")," · Bant ",TEXT(B14,"₺ #,##0")),"-")',
      yazi=DIS_REF_YEŞIL, kaydir=True)
    ws.merge_cells("B15:H15")

    h(ws, 17, 1, "Duyarlılık (tornado)", kalin=True, boyut=12, yazi=KOYU_LACIVERT)
    h(ws, 18, 1, "Değişken", kalin=True, yazi="FFFFFF", zemin=ORTA_MAVI)
    h(ws, 18, 2, "Etki [₺]", kalin=True, yazi="FFFFFF", zemin=ORTA_MAVI)
    h(ws, 18, 3, "Sıra", kalin=True, yazi="FFFFFF", zemin=ORTA_MAVI)
    h(ws, 19, 1, "Tornado matrah etkisi", yazi="333333")
    h(ws, 19, 2, "=IFERROR(MOTOR!G49,0)", sayi=TL, yazi=DIS_REF_YEŞIL)
    h(ws, 20, 1, "Tornado mevcut etkisi", yazi="333333")
    h(ws, 20, 2, "=IFERROR(MOTOR!G50,0)", sayi=TL, yazi=DIS_REF_YEŞIL)
    h(ws, 21, 1, "Tornado oran etkisi", yazi="333333")
    h(ws, 21, 2, "=IFERROR(MOTOR!G51,0)", sayi=TL, yazi=DIS_REF_YEŞIL)
    h(ws, 22, 1, "Tornado matris seti", yazi="333333")
    h(ws, 22, 2, "=IFERROR(ABS(MATRIS!R6-tbo_kacirilan),0)", sayi=TL, yazi=DIS_REF_YEŞIL)
    h(ws, 19, 3, '=IFERROR(IF(B19>=MAX(B19:B22),1,IF(B19>=MEDIAN(B19:B22),2,IF(B19>=MIN(B19:B22),3,4))),4)', sayi=CATI, yazi=DIS_REF_YEŞIL)
    h(ws, 20, 3, '=IFERROR(IF(B20>=MAX(B19:B22),1,IF(B20>=MEDIAN(B19:B22),2,IF(B20>=MIN(B19:B22),3,4))),4)', sayi=CATI, yazi=DIS_REF_YEŞIL)
    h(ws, 21, 3, '=IFERROR(IF(B21>=MAX(B19:B22),1,IF(B21>=MEDIAN(B19:B22),2,IF(B21>=MIN(B19:B22),3,4))),4)', sayi=CATI, yazi=DIS_REF_YEŞIL)
    h(ws, 22, 3, '=IFERROR(IF(B22>=MAX(B19:B22),1,IF(B22>=MEDIAN(B19:B22),2,IF(B22>=MIN(B19:B22),3,4))),4)', sayi=CATI, yazi=DIS_REF_YEŞIL)
    h(ws, 23, 1, "Duyarlılık sıra özeti", kalin=True, yazi=KOYU_LACIVERT)
    h(ws, 23, 2, '=IFERROR(CONCATENATE("1)",INDEX(A19:A22,MATCH(1,C19:C22,0))," 2)",INDEX(A19:A22,MATCH(2,C19:C22,0))),"-")',
      yazi=DIS_REF_YEŞIL)
    h(ws, 24, 1, "Duyarlılık yorumu", kalin=True, yazi=KOYU_LACIVERT)
    h(ws, 24, 2,
      '=_xlfn.TEXTJOIN("; ",TRUE,"En yüksek etki ",TEXT(MAX(B19:B22),"₺ #,##0")," TL — öncelik bu değişkende")',
      yazi=DIS_REF_YEŞIL, kaydir=True)

    h(ws, 26, 1, "M-SEN kural değişimi kaçırılan", kalin=True, yazi=KOYU_LACIVERT)
    h(ws, 26, 2, "=D9", sayi=TL, yazi=DIS_REF_YEŞIL)
    h(ws, 27, 1, "M-SEN yorumu", kalin=True, yazi=KOYU_LACIVERT)
    h(ws, 27, 2,
      '=_xlfn.TEXTJOIN("; ",TRUE,"5 puan oranı +1 puan senaryosunda kaçırılan ",TEXT(D9,"₺ #,##0"),'
      '" TL · Matris seti kaçırılan ",TEXT(D12,"₺ #,##0"))',
      yazi=DIS_REF_YEŞIL, kaydir=True)

    h(ws, 29, 1, "Tahmin aralığı (kaçırılan)", kalin=True, yazi=KOYU_LACIVERT)
    h(ws, 29, 2, "=MOTOR!G48", sayi=TL, yazi=DIS_REF_YEŞIL)
    h(ws, 29, 3, "=tbo_kacirilan", sayi=TL, yazi=DIS_REF_YEŞIL)
    h(ws, 29, 4, "=MOTOR!G47", sayi=TL, yazi=DIS_REF_YEŞIL)
    h(ws, 30, 1, "Tahmin (FORECAST)", kalin=True, yazi=KOYU_LACIVERT)
    h(ws, 31, 1, "Seri nokta", yazi=GRİ)
    for i, v in enumerate([1, 2, 3, 4, 5, 6, 7], 32):
        h(ws, i, 1, v, sayi=CATI)
        h(ws, i, 2, f"=D{5+v}", sayi=TL, yazi=DIS_REF_YEŞIL)
    h(ws, 40, 1, "Tahmin sonraki", yazi="333333")
    h(ws, 40, 2, "=IFERROR(FORECAST.LINEAR(8,B32:B38,A32:A38),0)", sayi=TL, yazi=DIS_REF_YEŞIL)
    h(ws, 41, 1, "Tahmin yorumu", kalin=True, yazi=KOYU_LACIVERT)
    h(ws, 41, 2,
      '=_xlfn.TEXTJOIN("; ",TRUE,"Tahmin ",TEXT(B40,"₺ #,##0")," · Alt ",TEXT(B29,"₺ #,##0"),'
      '" · Üst ",TEXT(D29,"₺ #,##0"))',
      yazi=DIS_REF_YEŞIL, kaydir=True)

    h(ws, 5, 7, "KacirilanDemo", kalin=True, yazi="FFFFFF", zemin=ORTA_MAVI)
    for i, v in enumerate([18800, 22000, 26400, 23500, 20000, 25000, 21000], 6):
        h(ws, i, 7, v, sayi=TL, yazi=GRİ)
    h(ws, 18, 5, "EtkiDemo", kalin=True, yazi="FFFFFF", zemin=ORTA_MAVI)
    for i, v in enumerate([1600, 1100, 1500, 900], 19):
        h(ws, i, 5, v, sayi=TL, yazi=GRİ)
    h(ws, 31, 4, "TrendDemo", kalin=True, yazi=KOYU_LACIVERT)
    for i, v in enumerate([18800, 22000, 26400, 23500, 20000, 25000, 21000], 32):
        h(ws, i, 4, v, sayi=TL, yazi=GRİ)

    g1 = BarChart()
    g1.type = "col"
    g1.title = "Senaryo Kaçırılan Karşılaştırması"
    g1.add_data(Reference(ws, min_col=7, min_row=5, max_row=12), titles_from_data=True)
    g1.set_categories(Reference(ws, min_col=1, min_row=6, max_row=12))
    ...
    g2.add_data(Reference(ws, min_col=5, min_row=18, max_row=22), titles_from_data=True)
    g2.set_categories(Reference(ws, min_col=1, min_row=19, max_row=22))
    g2.height, g2.width = 8, 12
    ws.add_chart(g2, "F18")

    g3 = LineChart()
    g3.title = "Senaryo Kaçırılan Trendi"
    g3.add_data(Reference(ws, min_col=4, min_row=31, max_row=38), titles_from_data=True)
    g3.set_categories(Reference(ws, min_col=1, min_row=32, max_row=38))
    g3.height, g3.width = 8, 12
    ws.add_chart(g3, "F32")

    genislik(ws, {"A": 55, "B": 18, "C": 18, "D": 18, "E": 22, "G": 14})
...
def vakalar(ws):
    ...
    vakalar_data = [
        ("V-001", "Tebliğ örnek 1 — 5 puan",
         ...
         15000,
         "=IFERROR(ROUND(vaka_matrah_1*INDEX(tblKurallar[deger_2025],MATCH(\"TBO-001\",tblKurallar[kural_id],0)),2),0)",
         "=IFERROR(D6-E6,0)",
         '=IFERROR(IF(ABS(F6)<=tbo_tolerans,"TUTARLI","KIRIK"),"KIRIK")',
         "5510 md.81 örnek"),
        ("V-002", "Tebliğ örnek 2 — ilave istihdam",
         "ilave_matrah=vaka_matrah_2",
         14500,
         "=IFERROR(ROUND(vaka_matrah_2*INDEX(tblKurallar[deger_2025],MATCH(\"TBO-002\",tblKurallar[kural_id],0)),2),0)",
         "=IFERROR(D7-E7,0)",
         '=IFERROR(IF(ABS(F7)<=tbo_tolerans,"TUTARLI","KIRIK"),"KIRIK")',
         "4447 geç.10 örnek"),
        ("V-003", "Tebliğ örnek 3 — birikimli A",
         "5 puan + ilave",
         29500,
         "=IFERROR(ROUND(vaka_matrah_1*INDEX(tblKurallar[deger_2025],MATCH(\"TBO-001\",tblKurallar[kural_id],0))"
         "+vaka_matrah_2*INDEX(tblKurallar[deger_2025],MATCH(\"TBO-002\",tblKurallar[kural_id],0)),2),0)",
         "=IFERROR(D8-E8,0)",
         '=IFERROR(IF(ABS(F8)<=tbo_tolerans,"TUTARLI","KIRIK"),"KIRIK")',
         "Birikim yorum A"),
        ("V-004", "Tavan × 5 puan kontrol",
         "tavan=vaka_tavan; oran TBO-001",
         7500,
         "=IFERROR(ROUND(vaka_tavan*INDEX(tblKurallar[deger_2025],MATCH(\"TBO-001\",tblKurallar[kural_id],0)),2),0)",
         "=IFERROR(D9-E9,0)",
         '=IFERROR(IF(ABS(F9)<=tbo_tolerans,"TUTARLI","KIRIK"),"KIRIK")',
         "TBO-003 tavan kontrol"),
    ]
    ...
    formuller = {
        ...
        "durum": '=IF(tblVakalar[[#This Row],[vaka_id]]="","",IFERROR(IF(ABS(tblVakalar[[#This Row],[fark]])<=tbo_tolerans,"TUTARLI","KIRIK"),"KIRIK"))',
    }
...
def kanit_raporu(ws):
    ...
    h(ws, 3, 1, "KANIT RAPORU — Teşvikli Bordro Optimizasyon", kalin=True, boyut=14, yazi=KOYU_LACIVERT, kaydir=True)
    ...
    for i, (et, form, sayi) in enumerate([
        ...
        ("Mevcut teşvik", "=tbo_mevcutTesvik", TL),
    ], 9):
    ...
    h(ws, 18, 2, "=tbo_potansiyelTasarruf", sayi=TL, yazi=DIS_REF_YEŞIL, kalin=True)
    h(ws, 19, 1, "Kaçırılan", yazi="333333")
    h(ws, 19, 2, "=tbo_kacirilan", sayi=TL, yazi=DIS_REF_YEŞIL, kalin=True)
    h(ws, 20, 1, "KARAR", kalin=True, yazi=KOYU_LACIVERT)
    h(ws, 20, 2, "=tbo_kararMetni", kalin=True, boyut=12, yazi=DIS_REF_YEŞIL)
    h(ws, 21, 1, "Gerekçe", yazi="333333")
    h(ws, 21, 2, "=tbo_kararGerekce", yazi=DIS_REF_YEŞIL, kaydir=True)
    ws.merge_cells("B21:F21")

    h(ws, 23, 1, "Madde atıfları", kalin=True, yazi=KOYU_LACIVERT)
    h(ws, 23, 2, "=tbo_maddeAtifMetni", yazi=DIS_REF_YEŞIL, kaydir=True)
    ws.merge_cells("B23:F23")

    h(ws, 25, 1, "Kanıt gövdesi", kalin=True, yazi=KOYU_LACIVERT)
    h(ws, 25, 2, "=tbo_kanitRaporu", yazi=DIS_REF_YEŞIL, kaydir=True)
    ws.merge_cells("B25:F25")

    h(ws, 27, 1, "Parmak izi (SHA-256 kısaltma)", yazi="333333")
    h(ws, 27, 2, "=tbo_parmakIzi", yazi=GRİ)
...
def pano(ws):
    ...
    h(ws, 3, 1, "Yönetim paneli — Teşvikli Bordro Optimizasyon", kalin=True, boyut=14, yazi=KOYU_LACIVERT)
    ws.column_dimensions["A"].width = 48

    kpiler = [
        (2, "Matrah", "=IFERROR(MOTOR!G21,0)", TL),
        (3, "Potansiyel", "=IFERROR(tbo_potansiyelTasarruf,0)", TL),
        (4, "Mevcut", "=IFERROR(tbo_mevcutTesvik,0)", TL),
        (5, "Kaçırılan", "=IFERROR(tbo_kacirilan,0)", TL),
        ...
        (11, "M-SEN", "=IFERROR(tbo_kuralDegisimSenaryo,0)", TL),
        (12, "Tahmin", "=IFERROR(SENARYO!B40,0)", TL),
        (13, "Vaka", "=IFERROR(tbo_vakaDurum,\"-\")", None),
    ]
    ...
    h(ws, 6, 2, "=tbo_kararMetni", boyut=14, kalin=True, yazi=DIS_REF_YEŞIL)
    h(ws, 7, 1, "Gerekçe", kalin=True, yazi=KOYU_LACIVERT)
    h(ws, 7, 2, "=tbo_kararGerekce", yazi=DIS_REF_YEŞIL, kaydir=True)
    ...
    moduller = [
        ...
        ("O1", "Kaçırılan sapması",
         "=IFERROR(IF(COUNT(SENARYO!D6:D12)<2,0,STDEV.P(SENARYO!D6:D12)),0)",
         '=_xlfn.TEXTJOIN("; ",TRUE,"Senaryo kaçırılan sapması ",TEXT(C13,"₺ #,##0")," TL")'),
        ("O2", "Tornado max etki",
         "=IFERROR(tbo_duyarlilikSira,\"-\")",
         "=IFERROR(tbo_yorumDuyarlilik,\"-\")"),
        ("O3", "Senaryo bant",
         "=IFERROR(tbo_senaryoKarsilastirma,0)",
         "=IFERROR(tbo_yorumSenaryo,\"-\")"),
        ("O6", "Kod payı",
         "=IFERROR(IF(tbo_potansiyelTasarruf=0,0,MOTOR!G22/tbo_potansiyelTasarruf),0)",
         '=_xlfn.TEXTJOIN("; ",TRUE,"5 puan / potansiyel ",TEXT(C16,"0.0%")," · Matris ",MATRIS!R7)'),
        ("O8", "Kalite skoru",
         "=IFERROR(KONTROLLER!B12,0)",
         '=_xlfn.TEXTJOIN("; ",TRUE,"Kalite ",TEXT(C17,"0")," puan")'),
        ("M", "M-SEN kaçırılan",
         "=IFERROR(tbo_kuralDegisimSenaryo,0)",
         "=IFERROR(tbo_yorumKuralDegisim,\"-\")"),
        ("I1", "Tahmin aralık",
         "=IFERROR(tbo_tahminAralik,0)",
         "=IFERROR(tbo_yorumTahmin,\"-\")"),
        ("I2", "Senaryo kaçırılan P90",
         "=IFERROR(IF(COUNT(SENARYO!D6:D12)<2,0,_xlfn.PERCENTILE.INC(SENARYO!D6:D12,tbo_yuzdelikOran)),0)",
    ...
    genislik(ws, {"A": 48, "B": 16, "C": 18, "D": 55})
...
def kontroller(ws):
    ...
    kontroller_list = [
        ("Giriş tablosu boş mu?",
         '=IF(COUNTA(tblBordro[CalisanNo])=0,"BOŞ","DOLU")'),
        ("Negatif brüt matrah?",
         '=IF(GIRDI!B9<0,"NEGATİF","TEMİZ")'),
        ("Hesap yılı geçerli mi?",
         '=IFERROR(IF(AND(N(hesapYili)>N(tbo_yilTaban),N(hesapYili)<=N(tbo_yilTaban)+3),"TEMİZ","HATALI"),"HATALI")'),
        ("Matrah sıfır mı?",
         '=IF(IFERROR(MOTOR!G21,0)=0,"SIFIR","NORMAL")'),
        ("Vaka kırık mı?",
         '=IFERROR(IF(COUNTIF(tblVakalar[durum],"KIRIK")>0,"KIRIK","TUTARLI"),"KIRIK")'),
        ("Kaçırılan eşiği aşıyor mu?",
         '=IF(IFERROR(tbo_kacirilan,0)>tbo_kacirilanEsik,"AŞIYOR","NORMAL")'),
        ("Yorum A/B seçili mi?",
    ...
    h(ws, 14, 2, "=IFERROR(tbo_girisBeklenen+COUNTA(GIRDI!B6:B16)*0,0)", sayi=CATI, yazi=DIS_REF_YEŞIL)
    ...
    h(ws, 17, 2, "=IFERROR(tbo_potansiyelTasarruf,0)", sayi=TL, yazi=DIS_REF_YEŞIL)
    h(ws, 18, 1, "Kaçırılan kontrol", yazi="333333")
    h(ws, 18, 2, "=IFERROR(tbo_kacirilan,0)", sayi=TL, yazi=DIS_REF_YEŞIL)
    h(ws, 19, 1, "Karar kontrol", yazi="333333")
    h(ws, 19, 2, "=IFERROR(tbo_kararMetni,\"-\")", yazi=DIS_REF_YEŞIL)
...
def ornek_veri(ws):
    ...
    formuller = {
        "Potansiyel": (
            ...
            "MATCH(\"TBO-001\",tblKurallar[kural_id],0))"
            "+tblOrnek[[#This Row],[İlave]]*INDEX(tblKurallar[deger_2025],"
            "MATCH(\"TBO-002\",tblKurallar[kural_id],0))"
            "+tblOrnek[[#This Row],[Kadın/Genç]]*INDEX(tblKurallar[deger_2025],"
            "MATCH(\"TBO-004\",tblKurallar[kural_id],0))"
            "+tblOrnek[[#This Row],[Engelli]]*tbo_isverenOran,2),0))"
        ),
        "Kaçırılan": (
            "=IF(tblOrnek[[#This Row],[Senaryo]]=\"\",\"\","
...
def listeler(ws):
    ...
    h(ws, 5, 7, "Teşvik kodu", kalin=True, yazi="FFFFFF", zemin=ORTA_MAVI)
    for i, v in enumerate(["TBO-001", "TBO-002", "TBO-004", "TBO-ENG", "YOK"], 6):
        h(ws, i, 7, v, zemin=GIRIS_SARI)
        ws.cell(i, 7).protection = Protection(locked=False)
        _kim(ws, f"G{i}", "Kod", "Mevcut veya avantajlı teşvik kodu")
...
def ayarlar(ws):
    ...
    params = [
        ...
        ("tbo_tolerans", 0.01, "TL", "Vaka tutarlılık toleransı", "Uygulama notu", "01.01.2025", "11.08.2026"),
        ("tbo_kacirilanEsik", 15_000, "TL", "PANO kaçırılan uyarı eşiği", "İç politika", "01.01.2025", "11.08.2026"),
        ("tbo_senaryoIyi", 0.90, "çarpan", "İyimser matrah çarpanı", "Model varsayımı", "01.01.2025", "11.08.2026"),
        ("tbo_senaryoKotu", 1.10, "çarpan", "Kötümser matrah çarpanı", "Model varsayımı", "01.01.2025", "11.08.2026"),
        ("tbo_oranArtisPuan", 0.01, "oran", "M-SEN +1 puan", "Senaryo motoru", "01.01.2025", "11.08.2026"),
        ("tbo_tahminAlt", 0.85, "çarpan", "Tahmin alt bant", "Model varsayımı", "01.01.2025", "11.08.2026"),
        ("tbo_tahminUst", 1.15, "çarpan", "Tahmin üst bant", "Model varsayımı", "01.01.2025", "11.08.2026"),
        ("tbo_tornadoMatrah", 1.05, "çarpan", "Tornado matrah şoku", "Model varsayımı", "01.01.2025", "11.08.2026"),
        ("tbo_tornadoMevcut", 0.95, "çarpan", "Tornado mevcut şoku", "Model varsayımı", "01.01.2025", "11.08.2026"),
        ("tbo_isverenOran", 0.205, "oran", "İşveren SGK payı (engelli)", "5510 s.K.", "01.01.2025", "11.08.2026"),
        ("tbo_yuzdelikOran", 0.90, "oran", "Yüzdelik P90", "İstatistik", "01.01.2025", "11.08.2026"),
        ("tbo_parmakIzi", "TBO-PRO-1.0.0", "metin", "Dosya parmak izi etiketi", "Üretim", "01.01.2025", "11.08.2026"),
        ("dosya_surumu", SURUM, "metin", "Ürün sürümü", "ExcelArşiv", "01.01.2025", "11.08.2026"),
        ("tbo_girisBeklenen", 11, "adet", "Zorunlu giriş alanı sayısı", "Ürün mimarisi", "01.01.2025", "11.08.2026"),
        ("tbo_riskDusuk", 15, "puan", "Düşük risk skoru", "İç politika", "01.01.2025", "11.08.2026"),
        ("tbo_riskOrta", 55, "puan", "Orta risk skoru", "İç politika", "01.01.2025", "11.08.2026"),
        ("tbo_riskYuksek", 85, "puan", "Yüksek risk skoru", "İç politika", "01.01.2025", "11.08.2026"),
        ("tbo_riskEsikOran", 0.5, "oran", "Yüksek risk kaçırılan/potansiyel", "İç politika", "01.01.2025", "11.08.2026"),
        ("tbo_yilTaban", 2024, "yıl", "CHOOSE yıl tabanı (hesapYili−taban)", "Ürün mimarisi", "01.01.2025", "11.08.2026"),
        ("tbo_yuvarMax", 10, "adet", "ROUND basamak tavanı", "Ürün mimarisi", "01.01.2025", "11.08.2026"),
        ("tbo_azamiMatrahEsik", 100_000_000, "TL", "Uç matrah eşiği (Ö2)", "İç politika", "01.01.2025", "11.08.2026"),
...
def kilavuz(ws):
    ...
    for i, m in enumerate([
        "VERİ YOK — BORDRO satırı yoksa hesap yapılmaz.",
        "UYGUN — mevcut teşvik ≥ potansiyel (kaçırılan = 0).",
        "İNCELE — kaçırılan > 0 ama eşik altında; gözden geçirin.",
        "UYGULA — kaçırılan ≥ eşik; MATRIS kişi×kod setini uygulayın.",
        "MATRIS — her kişi için TBO-001/002/004/ENG tutarlarını yan yana görüp en iyi seti kilitler.",
        "SENARYO — iyimser/baz/kötümser + M-SEN + agresif/muhafazakar + matris seti.",
    ], 5):
        h(ws, i, 1, "• " + m, yazi="333333", kaydir=True)
        ws.merge_cells(start_row=i, start_column=1, end_row=i, end_column=20)

    h(ws, 12, 1, "Belirsizlik beyanları (M05)", kalin=True, boyut=12, yazi=KRITIK)
    for i, m in enumerate(belirsizlik_beyanlari(["tesvik_birikim_sirasi"]), 13):
    ...
    h(ws, 19, 1,
      "GIRDI → BORDRO → MATRIS (kişi×kod) → KURALLAR → MOTOR → SENARYO → VAKALAR → PANO → KANIT_RAPORU → AYARLAR.",
      kaydir=True, yazi="333333")
...
def kosullu_bicimlendirme(wb):
    ...
    ws.conditional_formatting.add("B6", FormulaRule(formula=['ISNUMBER(SEARCH("UYGULA",B6))'], font=amber, fill=a_fill))
    ...
    ws.conditional_formatting.add("E6:E9", FormulaRule(formula=['OR(ISNUMBER(SEARCH("UYGULA",E6)),ISNUMBER(SEARCH("İNCELE",E6)))'], font=amber))
    ...
    ws.conditional_formatting.add("F7:F53", FormulaRule(formula=['F7="TBO-001"'], font=Font(color="B08948", bold=True)))
    ws.conditional_formatting.add("F7:F53", FormulaRule(formula=['F7="TBO-005"'], font=Font(color=KRITIK, bold=True)))

    ws = wb["BORDRO"]
    ws.conditional_formatting.add("L7:L56", CellIsRule(operator="greaterThan", formula=["0"], font=kirmizi))
    ws.conditional_formatting.add("L7:L56", CellIsRule(operator="equal", formula=["0"], font=yesil))
    ws.conditional_formatting.add("I7:I56", FormulaRule(formula=['I7="YOK"'], font=amber))
    ws.conditional_formatting.add("I7:I56", FormulaRule(formula=['AND(I7<>"",I7<>"YOK",I7<>H7)'], font=kirmizi, fill=a_fill))
    ...
    ws.conditional_formatting.add("B20", FormulaRule(formula=['ISNUMBER(SEARCH("UYGULA",B20))'], font=amber, fill=a_fill))
...
def ad_tanimlari(wb):
    ...
    ad_ekle(wb, "tbo_oranBesPuan", "MOTOR!$G$8")
    ad_ekle(wb, "tbo_oranIlave", "MOTOR!$G$9")
    ad_ekle(wb, "tbo_oranKadinGenc", "MOTOR!$G$11")
    ad_ekle(wb, "tbo_mevcutTesvik", "MOTOR!$G$19")
    ad_ekle(wb, "tbo_potansiyelTasarruf", "MOTOR!$G$28")
    ad_ekle(wb, "tbo_kacirilan", "MOTOR!$G$30")
    ad_ekle(wb, "tbo_kararMetni", "MOTOR!$G$34")
    ad_ekle(wb, "tbo_kararGerekce", "MOTOR!$G$35")
    ad_ekle(wb, "tbo_maddeAtifMetni", "MOTOR!$G$52")
    ad_ekle(wb, "tbo_kanitRaporu", "MOTOR!$G$53")
    ad_ekle(wb, "tbo_kuralYilMatris", "KURALLAR!$B$17")
    ad_ekle(wb, "tbo_parmakIzi", "AYARLAR!$B$19")

    ad_ekle(wb, "tbo_senaryoKarsilastirma", "SENARYO!$B$14")
    ad_ekle(wb, "tbo_yorumSenaryo", "SENARYO!$B$15")
    ad_ekle(wb, "tbo_duyarlilikSira", "SENARYO!$B$23")
    ad_ekle(wb, "tbo_yorumDuyarlilik", "SENARYO!$B$24")
    ad_ekle(wb, "tbo_kuralDegisimSenaryo", "SENARYO!$B$26")
    ad_ekle(wb, "tbo_yorumKuralDegisim", "SENARYO!$B$27")
    ad_ekle(wb, "tbo_tahminAralik", "SENARYO!$B$40")
    ad_ekle(wb, "tbo_yorumTahmin", "SENARYO!$B$41")
    ad_ekle(wb, "tbo_kisiKodMatris", "MATRIS!$R$7")

    ad_ekle(wb, "tbo_vakaDurum", "VAKALAR!$B$12")
    ad_ekle(wb, "tbo_vakaFark", "VAKALAR!$B$13")
    ...
    ayar_map = {
        ...
        "tbo_senaryoIyi": 10,
        "tbo_senaryoKotu": 11,
        "tbo_oranArtisPuan": 12,
        "tbo_tahminAlt": 13,
        "tbo_tahminUst": 14,
        "tbo_tornadoMatrah": 15,
        "tbo_tornadoMevcut": 16,
        "tbo_isverenOran": 17,
        "tbo_yuzdelikOran": 18,
        "tbo_girisBeklenen": 21,
        "tbo_riskDusuk": 22,
        "tbo_riskOrta": 23,
        "tbo_riskYuksek": 24,
        "tbo_riskEsikOran": 25,
    ...
    modul_map = {
        "tbo_modulT1": (11, "C"), "tbo_modulT1Yorum": (11, "D"),
        "tbo_modulT2": (12, "C"), "tbo_modulT2Yorum": (12, "D"),
        "tbo_modulO1": (13, "C"), "tbo_modulO1Yorum": (13, "D"),
        "tbo_modulO6": (16, "C"), "tbo_modulO6Yorum": (16, "D"),
        "tbo_modulO8": (17, "C"), "tbo_modulO8Yorum": (17, "D"),
        "tbo_modulI2": (20, "C"), "tbo_modulI2Yorum": (20, "D"),
    }
    ...
    ad_ekle(wb, "ListeTesvikKod", "LISTELER!$G$6:$G$10")

    # D02 / rakip ayrım karşılıkları (SPEC karsilik birebir)
    ad_ekle(wb, "tbo_bordroSatir", "BORDRO!$A$6")
    ad_ekle(wb, "tbo_enAvantajli", "MATRIS!$R$5")
    ad_ekle(wb, "tbo_kararKapi", "MOTOR!$G$34")
...
def main(cikti_yolu=None):
    ...
    tablo_formullerini_hucrelere_yaz(wb, satir_basi=6, satir_sonu=1006)
    ...
    varsayilan = os.path.join(kok, "TesvikliBordroOptimizasyon.xlsx")
...
if __name__ == "__main__":
    ...
    cikti = os.path.join(repo, "cikti", "TesvikliBordroOptimizasyon.xlsx")
