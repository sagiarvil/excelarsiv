...
# -*- coding: utf-8 -*-
"""
EXCELARŞİV DECISION OS v15 — UNIFIED SUITE
==========================================

Bu tek dosya iki sistemi birlikte taşır:

1) MASTER MANDATE / OPERATIONAL KERNEL
   - AI ve insan geliştirici için bağlayıcı üretim sözleşmesi.
   - Workbook'un nasıl tasarlanacağını, üretileceğini, test edileceğini,
     kanıtlanacağını ve ticari değerinin nasıl ölçüleceğini tanımlar.

2) OPERATIONAL RELEASE GATE
   - RELEASE_MANIFEST.json dosyasını bağımsız olarak doğrular.
   - Kanıtsız PASS, hard-failure ile FINAL, ticari kanıt olmadan
     COMMERCIAL/MARKET_VALIDATED statüsü verilmesini engeller.

ÇALIŞMA MODELİ
---------------
MANDATE → BUILD → TEST → EVIDENCE → RELEASE_MANIFEST.json → RELEASE GATE

Bu dosya tek başına bir workbook üretmez.
Mandate bölümünü üretim sözleşmesi olarak uygular; release verifier bölümü ise
oluşturulan manifestin tutarlılığını kontrol eder.

KOMUTLAR
--------
Mandate metnini ekrana yaz:
    python EXCELARSIV_V15_UNIFIED_SUITE.py --print-mandate

Mandate'i ayrı markdown dosyası olarak çıkar:
    python EXCELARSIV_V15_UNIFIED_SUITE.py --extract-mandate out.md
  python EXCELARSIV_V15_UNIFIED_SUITE.py --print-hardening-report
  python EXCELARSIV_V15_UNIFIED_SUITE.py --extract-hardening-report out.md

Release manifestini doğrula:
    python EXCELARSIV_V15_UNIFIED_SUITE.py --verify RELEASE_MANIFEST.json

Kısa kullanım:
    python EXCELARSIV_V15_UNIFIED_SUITE.py RELEASE_MANIFEST.json
"""
...
# ---------------------------------------------------------------------------
# v15.1 HARDENED OPERATIONAL KERNEL
# ---------------------------------------------------------------------------
...
SUITE_VERSION = "15.1.0"
KERNEL_NAME = "EXCELARSIV_DECISION_OS_HARDENED_KERNEL"

ALLOWED_STATUSES = {
    "NOT_APPLICABLE", "READY", "RUNNING", "PASS", "WARN", "FAIL", "BLOCKED",
    "EXTERNAL_ACCEPTANCE_REQUIRED"
}
HARD_PASS = {"PASS", "NOT_APPLICABLE"}
RELEASE_STATES = {
    "DRAFT", "ENGINEERING_PASS", "FINANCIAL_PASS", "UAT_REQUIRED",
    "COMMERCIAL_DEFENSIBLE", "MARKET_VALIDATED", "FINAL_RELEASE"
}

ALL_MODULES = [
    "suitability", "governance", "maker_checker", "ontology", "data_contracts",
    "semantic_contracts", "dependency_graph", "blast_radius", "financial_identities",
    "oracle", "golden_dataset", "metamorphic", "chaos", "data_observability",
    "model_observability", "confidence", "decision_engine", "playbooks",
    "human_approval", "reverse_stress", "scenario", "k3", "onboarding",
    "commercial_value", "market_validation", "ooxml_integrity"
]

CORE_ARTIFACTS = {
    "SPEC.md", "VALUE_MAP.md", "ONTOLOGY.csv", "DATA_CONTRACTS.csv",
    "SEMANTIC_CONTRACTS.csv", "DEPENDENCY_GRAPH.csv", "ASSUMPTION_REGISTER.csv",
    "CONTROL_CATALOG.csv", "TEST_MATRIX.csv", "ORACLE_RESULTS.csv",
    "GOLDEN_DATASET_RESULTS.csv", "RELEASE_MANIFEST.json", "RELEASE_REPORT.md",
    "CHANGELOG.md", "ROLLBACK_PLAN.md"
}

CSV_SCHEMAS: Dict[str, List[str]] = {
    "ONTOLOGY.csv": ["type", "id", "display_name", "key_properties", "owner", "state"],
    "DATA_CONTRACTS.csv": [
        "Field", "Type", "Required", "Nullable", "Allowed", "Range", "Unit", "FK",
        "Default", "Validation", "ErrorCode"
    ],
    "SEMANTIC_CONTRACTS.csv": [
        "KPI", "BusinessMeaning", "SSOT", "FormulaOwner", "Unit", "Period", "Sign",
        "Denominator", "ZeroBehavior", "NegativeBehavior", "Invariant", "DownstreamDecision"
    ],
    "DEPENDENCY_GRAPH.csv": ["From", "To", "Type", "Rule"],
    "ASSUMPTION_REGISTER.csv": [
        "ID", "Name", "Value", "Unit", "Source", "SourceDate", "Owner", "Confidence",
        "Materiality", "Sensitivity", "ReviewDue", "KillThreshold", "Downstream"
    ],
    "ORACLE_RESULTS.csv": [
        "TestID", "Engine", "Case", "Expected", "Actual", "Difference", "Tolerance", "Result"
    ],
    "GOLDEN_DATASET_RESULTS.csv": ["TestID", "Case", "Expected", "Actual", "Result"],
    "INVARIANT_RESULTS.csv": ["TestID", "Identity", "Expected", "Actual", "Difference", "Tolerance", "Result"],
    "METAMORPHIC_RESULTS.csv": ["TestID", "Rule", "Expected", "Actual", "Result"],
    "CHECKER_REPORT.csv": ["CheckID", "Area", "Severity", "Finding", "Status"]
}

MODULE_ARTIFACT_HINTS = {
    "suitability": ["SUITABILITY_DECISION.md"],
    "governance": ["SPEC.md"],
    "maker_checker": ["CHECKER_REPORT.md", "CHECKER_REPORT.csv"],
    "ontology": ["ONTOLOGY.csv"],
    "data_contracts": ["DATA_CONTRACTS.csv"],
    "semantic_contracts": ["SEMANTIC_CONTRACTS.csv"],
    "dependency_graph": ["DEPENDENCY_GRAPH.csv"],
    "financial_identities": ["INVARIANT_RESULTS.csv"],
    "oracle": ["ORACLE_RESULTS.csv"],
    "golden_dataset": ["GOLDEN_DATASET_RESULTS.csv"],
    "metamorphic": ["METAMORPHIC_RESULTS.csv"],
    "chaos": ["CHAOS_RESULTS.csv", "TEST_MATRIX.csv"],
    "data_observability": ["DATA_HEALTH.json", "DATA_HEALTH.csv"],
    "model_observability": ["MODEL_HEALTH.json", "MODEL_HEALTH.csv"],
    "confidence": ["CONFIDENCE_RESULTS.json", "CONFIDENCE_RESULTS.csv"],
    "decision_engine": ["DECISION_QUEUE.csv"],
    "playbooks": ["ACTION_PLAYBOOKS.csv"],
    "human_approval": ["APPROVAL_LOG.csv"],
    "reverse_stress": ["REVERSE_STRESS_RESULTS.csv"],
    "scenario": ["SCENARIO_RESULTS.csv"],
    "k3": ["K3_RESULTS.csv"],
    "onboarding": ["ONBOARDING_RESULTS.csv"],
    "commercial_value": ["COMMERCIAL_VALUE.json"],
    "market_validation": ["MARKET_VALIDATION.json"],
    "ooxml_integrity": ["OOXML_REPORT.json"]
}
...
class ModuleResult:
    ...
    def to_manifest(self) -> Dict[str, Any]:
        out = {"status": self.status, "evidence": self.evidence}
        if self.reason:
...
def _normalize_bool(v: Any) -> bool:
    if isinstance(v, bool):
        return v
    return str(v).strip().lower() in {"1", "true", "yes", "y", "evet"}
...
def run_suitability_engine(cfg: Dict[str, Any]) -> ModuleResult:
    s = cfg.get("suitability", {})
    score = 0
    reasons: List[str] = []

    if _num(s.get("users"), 1) <= 3:
        score += 2; reasons.append("few_users:+2")
    if _normalize_bool(s.get("heavy_financial_modeling")):
        score += 2; reasons.append("financial_modeling:+2")
    if _normalize_bool(s.get("scenario_planning_valuation")):
        score += 2; reasons.append("scenario_planning:+2")
    if _normalize_bool(s.get("offline_required")):
        score += 1; reasons.append("offline:+1")
    if _normalize_bool(s.get("excel_native_users")):
        score += 1; reasons.append("excel_native:+1")
    if _num(s.get("rows"), 0) < 100000:
        score += 1; reasons.append("rows_lt_100k:+1")

    penalties = [
        ("concurrent_multi_write", -3), ("row_level_security", -3),
        ("immutable_enterprise_audit_log", -3), ("realtime_streaming", -3),
        ("api_orchestration_heavy", -2), ("mission_critical_multi_user_sor", -4)
    ]
    for key, pts in penalties:
        if _normalize_bool(s.get(key)):
            score += pts; reasons.append(f"{key}:{pts}")
    if _num(s.get("transactions"), 0) > 500000:
        score -= 3; reasons.append("transactions_gt_500k:-3")

    if score >= 4:
        decision, status = "EXCEL_PRIMARY", "PASS"
    elif score >= 1:
        decision, status = "EXCEL_ANALYSIS_FRONTEND_ONLY", "WARN"
    else:
        decision, status = "EXCEL_PRIMARY_REJECTED", "FAIL"

    return ModuleResult(
        "suitability", status, "SUITABILITY_DECISION.md", decision,
        {"score": score, "decision": decision, "signals": reasons}
    )


def run_governance_engine(cfg: Dict[str, Any]) -> ModuleResult:
    g = cfg.get("governance", {})
    keys = ["FinancialImpact", "ExternalUse", "DecisionCriticality", "RegulatoryExposure", "Irreversibility", "DataSensitivity"]
    vals = {k: max(0, min(4, int(_num(g.get(k), 0)))) for k in keys}
    score = sum(vals.values())
    if score <= 5: tier = "T1"
    elif score <= 9: tier = "T2"
    elif score <= 14: tier = "T3"
    elif score <= 19: tier = "T4"
    else: tier = "T5"
    return ModuleResult("governance", "PASS", "SPEC.md", f"RiskScore={score};Tier={tier}", {"risk_score": score, "tier": tier, **vals})


def run_confidence_engine(cfg: Dict[str, Any]) -> ModuleResult:
    c = cfg.get("confidence", {})
    source = _num(c.get("SourceQuality"))
    completeness = _num(c.get("Completeness"))
    freshness = _num(c.get("Freshness"))
    reconciliation = _num(c.get("Reconciliation"))
    stability = _num(c.get("ModelStability"))
    score = 0.25*source + 0.20*completeness + 0.20*freshness + 0.20*reconciliation + 0.15*stability
    caps = []
    if _normalize_bool(c.get("missing_material_source")):
        score = min(score, 69); caps.append("missing_material_source_cap_69")
    if _normalize_bool(c.get("failed_reconciliation")):
        score = min(score, 49); caps.append("failed_reconciliation_cap_49")
    if _normalize_bool(c.get("stale_critical_source")):
        score = min(score, 49); caps.append("stale_critical_source_cap_49")
    if score >= 85: band = "HIGH"
    elif score >= 70: band = "MEDIUM"
    elif score >= 50: band = "LOW"
    else: band = "INSUFFICIENT"
    status = "PASS" if score >= 70 else ("WARN" if score >= 50 else "FAIL")
    return ModuleResult("confidence", status, "CONFIDENCE_RESULTS.json", band, {"score": round(score, 2), "band": band, "caps": caps})


def run_commercial_value_engine(cfg: Dict[str, Any]) -> ModuleResult:
    c = cfg.get("commercial_value", {})
    hours = _num(c.get("HoursSavedPerMonth"))
    hourly = _num(c.get("LoadedHourlyCost"))
    error = _num(c.get("ErrorCostAvoidedPerYear"))
    cash = _num(c.get("CashBenefitPerYear"))
    decision = _num(c.get("DecisionBenefitPerYear"))
    price = _num(c.get("Price"))
    annual_time = hours * 12 * hourly
    annual_value = annual_time + error + cash + decision
    capture = (price / annual_value) if annual_value > 0 else None
    mechanisms = sum(1 for x in [annual_time, error, cash, decision] if x > 0)
    gates = {
        "annual_value_ge_5000": annual_value >= 5000,
        "two_value_mechanisms": mechanisms >= 2,
        "customization_nontrivial": _normalize_bool(c.get("CustomizationNonTrivial")),
        "controls_audit_support": _normalize_bool(c.get("ControlsAuditSupport")),
        "demo_proves_value": _normalize_bool(c.get("DemoProvesValue")),
        "buyer_identifiable": _normalize_bool(c.get("BuyerIdentifiable")),
        "diy_analysis_complete": _normalize_bool(c.get("DIYAnalysisComplete")),
        "proof_pack_complete": _normalize_bool(c.get("ProofPackComplete")),
    }
    thousand_pass = all(gates.values())
    status = "PASS" if thousand_pass else "WARN"
    reason = "ECONOMICALLY_DEFENSIBLE_AT_GE_1000" if thousand_pass else "NOT_YET_DEFENSIBLE_AT_GE_1000"
    return ModuleResult("commercial_value", status, "COMMERCIAL_VALUE.json", reason, {
        "annual_time_value": round(annual_time, 2), "annual_quantified_value": round(annual_value, 2),
        "value_capture_ratio": None if capture is None else round(capture, 4), "mechanisms": mechanisms,
        "gates": gates
    })


def required_modules_for_tier(tier: str, cfg: Dict[str, Any]) -> List[str]:
    base = [
        "suitability", "governance", "ontology", "data_contracts", "semantic_contracts",
        "dependency_graph", "golden_dataset", "metamorphic", "chaos", "data_observability",
        "model_observability", "confidence", "decision_engine", "playbooks", "k3",
        "commercial_value", "market_validation", "ooxml_integrity"
    ]
    if _normalize_bool(cfg.get("financial_model", True)):
        base += ["financial_identities"]
    if tier in {"T3", "T4", "T5"}:
        base += ["oracle", "reverse_stress"]
    if tier in {"T4", "T5"}:
        base += ["maker_checker", "human_approval", "blast_radius"]
    if _normalize_bool(cfg.get("scenario_required", False)):
        base += ["scenario"]
    if _normalize_bool(cfg.get("onboarding_required", True)):
        base += ["onboarding"]
...
def validate_csv_schema(path: Path, required_columns: Iterable[str]) -> List[str]:
    errors: List[str] = []
    if not path.exists():
    ...
    try:
        ...
        return [f"CSV_READ_ERROR:{path.name}:{type(exc).__name__}"]
    missing = [c for c in required_columns if c not in headers]
    if missing:
        errors.append(f"CSV_MISSING_COLUMNS:{path.name}:{','.join(missing)}")
    if not rows:
        errors.append(f"CSV_EMPTY:{path.name}")
    return errors


def validate_dependency_graph(path: Path) -> Tuple[List[str], Dict[str, Any]]:
    errors = validate_csv_schema(path, CSV_SCHEMAS["DEPENDENCY_GRAPH.csv"])
    if errors:
        return errors, {"cycle_count": None}
    _, rows = _csv_rows(path)
    graph: Dict[str, List[str]] = {}
    for row in rows:
        a, b = row.get("From", "").strip(), row.get("To", "").strip()
        if a and b:
    ...
    for node in graph:
        if state.get(node, 0) == 0:
            dfs(node)
    if cycle_count:
        errors.append(f"DEPENDENCY_CYCLE_COUNT:{cycle_count}")
    return errors, {"cycle_count": cycle_count, "node_count": len(graph), "edge_count": sum(len(v) for v in graph.values())}


def validate_result_csv(path: Path, result_column: str = "Result") -> Tuple[List[str], Dict[str, Any]]:
    errors: List[str] = []
    if not path.exists():
        return [f"MISSING_FILE:{path.name}"], {"rows": 0, "failures": None}
    headers, rows = _csv_rows(path)
    if result_column not in headers:
        return [f"CSV_MISSING_COLUMNS:{path.name}:{result_column}"], {"rows": len(rows), "failures": None}
    failures = []
    for i, row in enumerate(rows, start=2):
        result = str(row.get(result_column, "")).strip().upper()
        if result not in {"PASS", "NOT_APPLICABLE", "N/A"}:
            failures.append(i)
    if failures:
        errors.append(f"RESULT_FAILURES:{path.name}:{len(failures)}")
    return errors, {"rows": len(rows), "failures": len(failures)}


def validate_artifact_set(project_dir: Path, tier: str, include_manifest: bool = True) -> Tuple[List[str], Dict[str, str]]:
    errors: List[str] = []
    hashes: Dict[str, str] = {}
    required = set(CORE_ARTIFACTS)
    if not include_manifest:
        required.discard("RELEASE_MANIFEST.json")
    if tier in {"T3", "T4", "T5"}:
        required.add("INVARIANT_RESULTS.csv")
    if tier in {"T4", "T5"}:
        if not ((project_dir / "CHECKER_REPORT.md").exists() or (project_dir / "CHECKER_REPORT.csv").exists()):
            errors.append("MISSING_ARTIFACT:CHECKER_REPORT")
    for name in sorted(required):
        p = project_dir / name
        if not p.exists():
            errors.append(f"MISSING_ARTIFACT:{name}")
        elif p.is_file() and name != "RELEASE_MANIFEST.json":
            # Manifest cannot hash itself without a recursive fixed-point problem.
            hashes[name] = _sha256_file(p)
    for name, cols in CSV_SCHEMAS.items():
        p = project_dir / name
        if p.exists():
            errors.extend(validate_csv_schema(p, cols))
    return errors, hashes


def scan_ooxml(workbook_path: Path) -> Tuple[List[str], Dict[str, Any]]:
    errors: List[str] = []
    metrics: Dict[str, Any] = {
        "path": str(workbook_path), "zip_ok": False, "xml_parse_errors": 0,
        "external_links": 0, "sheet_xml_count": 0, "formula_count": 0,
        "calc_mode": None, "full_calc_on_load": None, "force_full_calc": None
    }
    if not workbook_path.exists():
        return ["WORKBOOK_NOT_FOUND"], metrics
    try:
        with zipfile.ZipFile(workbook_path, "r") as zf:
            bad = zf.testzip()
            if bad:
                errors.append(f"ZIP_CRC_ERROR:{bad}")
            else:
                metrics["zip_ok"] = True
            names = zf.namelist()
            metrics["external_links"] = sum(1 for n in names if n.startswith("xl/externalLinks/") and n.endswith(".xml"))
            sheet_names = [n for n in names if re.fullmatch(r"xl/worksheets/sheet\d+\.xml", n)]
            metrics["sheet_xml_count"] = len(sheet_names)
            ...
            for n in names:
                ...
                try:
                    ...
                    if n == "xl/workbook.xml":
                        ...
                        if calc is not None:
                            metrics["calc_mode"] = calc.attrib.get("calcMode")
                            metrics["full_calc_on_load"] = calc.attrib.get("fullCalcOnLoad")
                            metrics["force_full_calc"] = calc.attrib.get("forceFullCalc")
                    if n in sheet_names:
                        metrics["formula_count"] += len(root.findall(".//main:f", ns))
                except ET.ParseError:
                    metrics["xml_parse_errors"] += 1
            if metrics["xml_parse_errors"]:
                errors.append(f"XML_PARSE_ERRORS:{metrics['xml_parse_errors']}")
    except zipfile.BadZipFile:
        errors.append("BAD_ZIP_XLSX")
    except Exception as exc:
        errors.append(f"OOXML_SCAN_ERROR:{type(exc).__name__}")
    return errors, metrics


# ----------------------------- MANIFEST GATE --------------------------------

def _resolve_evidence(project_dir: Path, evidence: str) -> Optional[Path]:
    if not evidence:
        return None
    p = Path(evidence)
...
def verify_manifest(path: str, strict: bool = True) -> Tuple[Dict[str, Any], int]:
    p = Path(path)
    data = _read_json(p)
    ...
    release = str(data.get("release_status", ""))
    if release not in RELEASE_STATES:
        errors.append(f"INVALID_RELEASE_STATUS:{release}")

    modules = data.get("modules")
    if not isinstance(modules, dict):
        modules = {}
        errors.append("MODULES_NOT_OBJECT")

    gov = modules.get("governance", {})
    tier = str(data.get("governance_tier") or gov.get("metrics", {}).get("tier") or "").upper()
    if tier not in {"T1", "T2", "T3", "T4", "T5"}:
        errors.append(f"INVALID_GOVERNANCE_TIER:{tier}")
        tier = "T5"  # fail-safe: enforce the strongest set if unknown

    cfg_path = project_dir / "SUITE_INPUT.json"
    cfg = _read_json(cfg_path) if cfg_path.exists() else {}
    required_modules = required_modules_for_tier(tier, cfg)
    ...
    for name in required_modules:
        ...
        if status not in ALLOWED_STATUSES:
            errors.append(f"INVALID_STATUS:{name}:{status}")
        if status in {"PASS", "WARN", "FAIL", "EXTERNAL_ACCEPTANCE_REQUIRED"} and not evidence:
            errors.append(f"MISSING_EVIDENCE:{name}")
        if evidence:
            ep = _resolve_evidence(project_dir, evidence)
            if ep is None:
                errors.append(f"EVIDENCE_PATH_ESCAPE:{name}:{evidence}")
            elif not ep.exists():
                errors.append(f"EVIDENCE_NOT_FOUND:{name}:{evidence}")

    hard_failures = data.get("hard_failures", [])
    if hard_failures:
        errors.append("HARD_FAILURES_PRESENT:" + ",".join(map(str, hard_failures)))

    artifact_errors, artifact_hashes = validate_artifact_set(project_dir, tier)
    errors.extend(artifact_errors)

    # Enforce hashes if manifest carries the evidence_hashes map.
    declared_hashes = data.get("evidence_hashes", {})
    if strict:
        if not isinstance(declared_hashes, dict) or not declared_hashes:
            errors.append("MISSING_EVIDENCE_HASHES")
        else:
            for name, actual in artifact_hashes.items():
                declared = declared_hashes.get(name)
                if declared is None:
                    errors.append(f"MISSING_HASH:{name}")
                elif str(declared).lower() != actual.lower():
                    errors.append(f"HASH_MISMATCH:{name}")

    # Mandatory result tables must have only pass-like outcomes.
    for result_file in ["ORACLE_RESULTS.csv", "GOLDEN_DATASET_RESULTS.csv", "METAMORPHIC_RESULTS.csv", "INVARIANT_RESULTS.csv"]:
        rp = project_dir / result_file
        if rp.exists():
            result_errors, _ = validate_result_csv(rp)
            errors.extend(result_errors)

    dep_path = project_dir / "DEPENDENCY_GRAPH.csv"
    if dep_path.exists():
        dep_errors, _ = validate_dependency_graph(dep_path)
        errors.extend(dep_errors)

    # FINAL_RELEASE is intentionally strict: WARN and external acceptance are not final.
    if release == "FINAL_RELEASE":
        for name in required_modules:
            st = str(modules.get(name, {}).get("status", ""))
            if st not in HARD_PASS:
                errors.append(f"FINAL_WITH_NONPASS:{name}:{st}")

    cv = modules.get("commercial_value", {})
    mv = modules.get("market_validation", {})
    if release in {"COMMERCIAL_DEFENSIBLE", "MARKET_VALIDATED", "FINAL_RELEASE"} and cv.get("status") != "PASS":
        errors.append("COMMERCIAL_STATUS_WITHOUT_COMMERCIAL_VALUE_PASS")
    if release in {"MARKET_VALIDATED", "FINAL_RELEASE"} and mv.get("status") != "PASS":
        errors.append("MARKET_STATUS_WITHOUT_MARKET_VALIDATION_PASS")

    if modules.get("k3", {}).get("status") == "EXTERNAL_ACCEPTANCE_REQUIRED" and release == "FINAL_RELEASE":
        errors.append("FINAL_WITH_EXTERNAL_K3_REQUIRED")
...
def _render_suitability_md(result: ModuleResult) -> str:
    m = result.metrics or {}
    return (
        "# Spreadsheet Suitability Decision\n\n"
        f"- Status: {result.status}\n- Decision: {result.reason}\n- Score: {m.get('score')}\n"
        f"- Signals: {', '.join(m.get('signals', []))}\n"
    )


def run_pipeline(project_dir: str) -> Tuple[Dict[str, Any], int]:
    root = Path(project_dir).resolve()
    cfg_path = root / "SUITE_INPUT.json"
    if not cfg_path.exists():
        raise SystemExit(f"SUITE_INPUT.json bulunamadı: {cfg_path}")
    cfg = _read_json(cfg_path)

    suitability = run_suitability_engine(cfg)
    governance = run_governance_engine(cfg)
    tier = governance.metrics["tier"] if governance.metrics else "T5"
    confidence = run_confidence_engine(cfg)
    commercial = run_commercial_value_engine(cfg)

    (root / "SUITABILITY_DECISION.md").write_text(_render_suitability_md(suitability), encoding="utf-8")
    _write_json(root / "CONFIDENCE_RESULTS.json", confidence.metrics or {})
    _write_json(root / "COMMERCIAL_VALUE.json", commercial.metrics or {})

    workbook = cfg.get("workbook_path")
    ooxml = ModuleResult("ooxml_integrity", "NOT_APPLICABLE", "", "No workbook configured", {})
    if workbook:
        wp = Path(workbook)
        if not wp.is_absolute():
            wp = root / wp
        ooxml_errors, ooxml_metrics = scan_ooxml(wp)
        _write_json(root / "OOXML_REPORT.json", {"errors": ooxml_errors, "metrics": ooxml_metrics})
        ooxml = ModuleResult("ooxml_integrity", "PASS" if not ooxml_errors else "FAIL", "OOXML_REPORT.json", ";".join(ooxml_errors), ooxml_metrics)

    # Read pre-existing module evidence. Runtime engines override the modules they actually compute.
    previous_manifest = {}
    ...
    if manifest_path.exists():
        try:
            ...
            previous_manifest = {}
    modules: Dict[str, Dict[str, Any]] = dict(previous_manifest.get("modules", {}))
    for r in [suitability, governance, confidence, commercial, ooxml]:
        modules[r.name] = r.to_manifest()

    required = required_modules_for_tier(tier, cfg)
    hard_failures: List[str] = []
    for name in required:
        obj = modules.get(name)
        if not isinstance(obj, dict):
            modules[name] = {"status": "BLOCKED", "evidence": "", "reason": "Required module has no runtime evidence"}
            hard_failures.append(f"MISSING_RUNTIME_MODULE:{name}")
        elif obj.get("status") in {"FAIL", "BLOCKED"}:
            hard_failures.append(f"MODULE_{obj.get('status')}:{name}")

    artifact_errors, artifact_hashes = validate_artifact_set(root, tier, include_manifest=False)
    hard_failures.extend(artifact_errors)

    release_status = "DRAFT"
    if not hard_failures:
        engineering_names = ["suitability", "governance", "ontology", "data_contracts", "semantic_contracts", "dependency_graph", "golden_dataset", "metamorphic", "chaos", "ooxml_integrity"]
        if all(modules.get(n, {}).get("status") in HARD_PASS for n in engineering_names if n in required):
            release_status = "ENGINEERING_PASS"
        financial_names = [n for n in ["financial_identities", "oracle", "reverse_stress"] if n in required]
        if release_status == "ENGINEERING_PASS" and all(modules.get(n, {}).get("status") in HARD_PASS for n in financial_names):
            release_status = "FINANCIAL_PASS"
        k3status = modules.get("k3", {}).get("status")
        if k3status == "EXTERNAL_ACCEPTANCE_REQUIRED":
            release_status = "UAT_REQUIRED"
        elif release_status == "FINANCIAL_PASS" and commercial.status == "PASS":
            release_status = "COMMERCIAL_DEFENSIBLE"
        if release_status == "COMMERCIAL_DEFENSIBLE" and modules.get("market_validation", {}).get("status") == "PASS":
            release_status = "MARKET_VALIDATED"
        if release_status == "MARKET_VALIDATED" and all(modules.get(n, {}).get("status") in HARD_PASS for n in required):
    ...
    manifest = {
        ...
        "governance_tier": tier,
        "compatibility_profile": cfg.get("compatibility_profile", ""),
        "modules": modules,
    ...
    verification, code = verify_manifest(str(manifest_path), strict=True)
    _write_json(root / "SUITE_VERIFICATION.json", verification)
    return {"manifest": manifest, "verification": verification}, code
...
def init_project(project_dir: str) -> Dict[str, Any]:
    ...
    if not cfg_path.exists():
        template = {
            ...
            "version": "0.1.0",
            "build_id": "",
            "compatibility_profile": "PROFILE-M",
            "financial_model": True,
            "scenario_required": False,
            "onboarding_required": True,
            "workbook_path": "",
            "suitability": {
                "users": 1, "rows": 10000, "transactions": 10000,
                "heavy_financial_modeling": True, "scenario_planning_valuation": True,
                "offline_required": True, "excel_native_users": True,
                "concurrent_multi_write": False, "row_level_security": False,
                "immutable_enterprise_audit_log": False, "realtime_streaming": False,
                "api_orchestration_heavy": False, "mission_critical_multi_user_sor": False
            },
            "governance": {
                "FinancialImpact": 2, "ExternalUse": 1, "DecisionCriticality": 2,
                "RegulatoryExposure": 0, "Irreversibility": 1, "DataSensitivity": 1
            },
            "confidence": {
                "SourceQuality": 90, "Completeness": 90, "Freshness": 90,
                "Reconciliation": 90, "ModelStability": 90,
                "missing_material_source": False, "failed_reconciliation": False,
                "stale_critical_source": False
            },
            "commercial_value": {
                "HoursSavedPerMonth": 0, "LoadedHourlyCost": 0,
                "ErrorCostAvoidedPerYear": 0, "CashBenefitPerYear": 0,
                "DecisionBenefitPerYear": 0, "Price": 1000,
                "CustomizationNonTrivial": False, "ControlsAuditSupport": False,
                "DemoProvesValue": False, "BuyerIdentifiable": False,
                "DIYAnalysisComplete": False, "ProofPackComplete": False
            }
        }
        _write_json(cfg_path, template)
    return {"project_dir": str(root), "suite_input": str(cfg_path)}
...
def print_usage():
    print(f"""EXCELARŞİV DECISION OS v{SUITE_VERSION} — HARDENED UNIFIED SUITE

Kullanım:
  python EXCELARSIV_V15_UNIFIED_SUITE.py --print-mandate
  python EXCELARSIV_V15_UNIFIED_SUITE.py --extract-mandate out.md
  python EXCELARSIV_V15_UNIFIED_SUITE.py --init-project <dir>
  python EXCELARSIV_V15_UNIFIED_SUITE.py --run <project_dir>
  python EXCELARSIV_V15_UNIFIED_SUITE.py --verify <RELEASE_MANIFEST.json>
  python EXCELARSIV_V15_UNIFIED_SUITE.py --scan-xlsx <workbook.xlsx>

Not:
  Bu kernel Excel GUI recalculation/K3 yerine geçmez. GUI erişimi yoksa K3,
  EXTERNAL_ACCEPTANCE_REQUIRED olarak kalmalıdır. Teknik release kanıtsız PASS üretmez.
""")
...
def main():
    ...
    if cmd == "--print-mandate":
        print(MANDATE.strip())
        return 0
    if cmd == "--extract-mandate":
        if len(args) != 2:
            raise SystemExit("usage: --extract-mandate out.md")
        out = Path(args[1])
        out.write_text(MANDATE.strip() + "\n", encoding="utf-8")
        print(str(out))
        return 0
    if cmd == "--print-hardening-report":
        print(HARDENING_REPORT.strip())
        return 0
    if cmd == "--extract-hardening-report":
        if len(args) != 2:
            raise SystemExit("usage: --extract-hardening-report out.md")
        out = Path(args[1])
        out.write_text(HARDENING_REPORT.strip() + "\n", encoding="utf-8")
        print(str(out))
        return 0
    if cmd == "--init-project":
        if len(args) != 2:
            raise SystemExit("usage: --init-project <dir>")
        print(json.dumps(init_project(args[1]), ensure_ascii=False, indent=2))
        return 0
    if cmd == "--run":
        if len(args) != 2:
            raise SystemExit("usage: --run <project_dir>")
        result, code = run_pipeline(args[1])
        print(json.dumps(result, ensure_ascii=False, indent=2))
        return code
    if cmd == "--verify":
        if len(args) != 2:
            raise SystemExit("usage: --verify RELEASE_MANIFEST.json")
        result, code = verify_manifest(args[1], strict=True)
        print(json.dumps(result, ensure_ascii=False, indent=2))
        return code
    if cmd == "--scan-xlsx":
        if len(args) != 2:
            raise SystemExit("usage: --scan-xlsx workbook.xlsx")
        errors, metrics = scan_ooxml(Path(args[1]))
        print(json.dumps({"result": "PASS" if not errors else "FAIL", "errors": errors, "metrics": metrics}, ensure_ascii=False, indent=2))
        return 0 if not errors else 1
    if len(args) == 1 and args[0].lower().endswith(".json"):
        result, code = verify_manifest(args[0], strict=True)
        print(json.dumps(result, ensure_ascii=False, indent=2))
        return code
    raise SystemExit("Geçersiz kullanım. --print-mandate, --extract-mandate, --print-hardening-report, --extract-hardening-report, --init-project, --run, --verify veya --scan-xlsx kullanın.")
