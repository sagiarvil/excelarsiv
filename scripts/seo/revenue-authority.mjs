...
const graph = { site: contract.site, version: contract.version, generatedAt: new Date().toISOString(), clusters: [] };

for (const c of contract.clusters || []) {
  const owner = norm(c.commercialOwner);
  if (!routes.has(owner)) fail(`${c.id}: commercial owner not in live registry: ${owner}`);
  if (!routes.has(norm(c.hub))) fail(`${c.id}: hub not in live registry: ${c.hub}`);
  if ((c.supportRoutes || []).length < contract.policy.minSupportRoutes) fail(`${c.id}: insufficient support routes`);
  for (const r of c.supportRoutes || []) if (!routes.has(norm(r))) fail(`${c.id}: support route missing: ${r}`);
  ...
  graph.clusters.push({
    ...
    owner,
    hub: norm(c.hub),
    supports: c.supportRoutes.map(norm),
    queries: c.ownedQueries,
    anchors: c.contextualAnchors,
    directAnswerQuestion: c.answer.question,
    evidence: c.evidence,
    conversion: c.conversion,
    lastMaterialReview: c.lastMaterialReview
  });
}
...
fs.writeFileSync(path.join(root, 'data', 'seo', 'authority', 'generated', 'contextual-link-plan.json'), JSON.stringify({
  generatedAt: graph.generatedAt,
  rules: contract.clusters.map(c => ({
    owner: norm(c.commercialOwner),
    sourceRoutes: [norm(c.hub), ...c.supportRoutes.map(norm)],
    anchors: c.contextualAnchors,
    insertionPolicy: 'first-meaningful-occurrence-only; never alter headings; max-one-link-per-anchor-per-page'
  }))
}, null, 2) + '\n');
